        ...
        if line == "beginseg":
            ...
            if directive == "name":
                ...
                include = SpecInclude(
                    ...
                    includeSectionsFlags=(
                        SpecIncludeSection.DATA
                        if directive == "include_data_only_within_rodata"
                        else (
                            (
                                SpecIncludeSection.TEXT
                                | SpecIncludeSection.RODATA
                                | SpecIncludeSection.BSS
                            )
                            if directive == "include_no_data"
                            else SpecIncludeSection.ALL
                        )
                    ),
                )
...
def stringify_spec(spec: Spec, indent=" " * 4):
    for segment in spec.segments:
        ...
        for include in segment.includes:
            if (
                include.dataWithRodata
                and include.includeSectionsFlags == SpecIncludeSection.DATA
            ):
                directive = "include_data_only_within_rodata"
                includeSectionsFlags_str = ""
            elif not include.dataWithRodata and include.includeSectionsFlags == (
                SpecIncludeSection.TEXT
                | SpecIncludeSection.RODATA
                | SpecIncludeSection.BSS
            ):
                directive = "include_no_data"
                includeSectionsFlags_str = ""
            else:
                directive = (
                ...
                if include.includeSectionsFlags == SpecIncludeSection.ALL:
                    ...
                    assert SpecIncludeSection.ALL not in include.includeSectionsFlags
...
def main():
    # cheat, use preprocessed spec from another repo
    base_spec_p = Path("../oot/build/gc-eu-mq-dbg/spec")
    assert base_spec_p.exists()
