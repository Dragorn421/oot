...
class AudioTableSample(AudioTableData):
    ...
    def __init__(
        self, start: int, end: int, header: SoundFontSample, data, book: AdpcmBook, loop: AdpcmLoop, padding=None
    ):
    ...
    def resolve_basenote_rate(self, extraction_sample_info: Dict[int, Dict[str, str]]):
        ...
        def update_tuning_map(tuning, rate, note):
            ...
            assert (
                ntuning == tuning or tuning_bits in BAD_FLOATS
        ...
        if len(self.notes_rates) == 1:
            ...
            if len(rate_cands) == 0:
                ...
                final_rate, final_notes = rank_rates_notes(
                    [(rate, tuple(D[rate] for D in dicts)) for rate in rate_cands]
                )
    ...
    def to_file(self, outpath: str):
        assert (
            self.sample_rate is not None and self.base_note is not None
        ...
        aifc.add_section(
            ...
            struct.pack(
                ...
                0,
                0,
                0,  # sustain(mode,start,end)
                0,
                0,
                0,  # release(mode,start,end)
            ),
        )
...
class AudioTableFile:
    ...
    def add_sample(self, sample_header: SoundFontSample, book: AdpcmBook, loop: AdpcmLoop, tuning: float, ob):
        ...
        if sample_start in self.samples:
            ...
            new_sample = AudioTableSample(
                sample_start, sample_end, sample_header, sample_data, book, loop, sample_padding
            )
                ...
                for j, bank in enumerate(all_sample_banks):
                    ...
                    for sample in bank.samples_final:
                        ...
                        if self.data[start:sample_end] == sample.data:
                            print(
                                f"    Located match for range [0x{start:X}:0x{sample_end:X}] in bank {j} at 0x{sample.start:X}"
                            )
                    ...
                    print(
                        f"    No match found in other banks for range [0x{start:X}:0x{end:X}], leaving as binary blob"
                    )
