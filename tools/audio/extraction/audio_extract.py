...
def collect_sample_banks(
    rom_image: memoryview,
    extracted_dir: str,
    version_info: GameVersionInfo,
    table: AudioCodeTable,
    ...
    for i, entry in enumerate(table):
        ...
        assert (
            entry.short_data1 == 0 and entry.short_data2 == 0 and entry.short_data3 == 0
        ), "Bad data for Sample Bank entry, all short data should be 0"
        assert entry.medium == AudioStorageMedium.MEDIUM_CART, "Bad data for Sample Bank entry, medium should be CART"
        ...
        if entry.size == 0:
            ...
            bank = AudioTableFile(
                ...
                rom_image,
                entry,
                version_info.audiotable_rom + table.rom_addr,
                buffer_bug=bug,
...
def collect_soundfonts(
    rom_image: memoryview,
    extracted_dir: str,
    version_info: GameVersionInfo,
    sound_font_table: AudioCodeTable,
    ...
    for i, entry in enumerate(sound_font_table):
        ...
        # Read the data
        soundfont = AudiobankFile(
            rom_image,
            i,
            entry,
            version_info.audiobank_rom + sound_font_table.rom_addr,
            bank1,
            bank2,
            entry.sample_bank_id_1,
            entry.sample_bank_id_2,
...
def extract_samplebank(extracted_dir: str, sample_banks: List[AudioTableFile], bank: AudioTableFile, write_xml: bool):
...
def extract_audio_for_version(version_name: str, rom_path: str, read_xml: bool, write_xml: bool):
    ...
    soundfonts = collect_soundfonts(
        rom_image, extracted_dir, version_info, sound_font_table, soundfont_xmls, sample_banks
    )
