...
def read_tables():
    ...
    baserom = None
    with open(f"baseroms/{oot_version}/baserom-decompressed.z64","rb") as infile:
        baserom = infile.read()
...
def dump_all_text():
    ...
    for i,entry in enumerate(combined_message_entry_table,0):
        if entry[0] == 0xFFFD:
            ...
            nes_text = ""
            with open(f"baseroms/{oot_version}/segments/nes_message_data_static","rb") as infile:
            ...
            if entry[0] != 0xFFFC:
                ...
                with open(f"baseroms/{oot_version}/segments/ger_message_data_static","rb") as infile:
                ...
                with open(f"baseroms/{oot_version}/segments/fra_message_data_static","rb") as infile:
...
def dump_staff_text():
    staff_message_data_static_size = path.getsize(f"baseroms/{oot_version}/segments/staff_message_data_static")
    ...
    for i,entry in enumerate(staff_message_entry_table,0):
        if entry[0] != 0xFFFF:
            ...
            with open(f"baseroms/{oot_version}/segments/staff_message_data_static","rb") as infile:
...
def main():
    ...
    parser.add_argument("--oot-version", required=True)
    parser.add_argument(
        "--nes_message_entry_table_addr", type=lambda s: int(s, 16), required=True
    )
    parser.add_argument(
        "--ger_message_entry_table_addr", type=lambda s: int(s, 16), required=True
    )
    parser.add_argument(
        "--fra_message_entry_table_addr", type=lambda s: int(s, 16), required=True
    )
    parser.add_argument(
        "--staff_message_entry_table_addr", type=lambda s: int(s, 16), required=True
    )
    parser.add_argument(
        "--staff_message_entry_table_addr_end", type=lambda s: int(s, 16), required=True
    )
    ...
    nes_message_entry_table_addr = args.nes_message_entry_table_addr
    ger_message_entry_table_addr = args.ger_message_entry_table_addr
    fra_message_entry_table_addr = args.fra_message_entry_table_addr
    staff_message_entry_table_addr = args.staff_message_entry_table_addr
    staff_message_entry_table_addr_end = args.staff_message_entry_table_addr_end
