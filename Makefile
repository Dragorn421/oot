...
ASFLAGS += -march=vr4300 -32 -no-pad-sections -I include -I$(EXTRACTED_DIR)
...
RELOC_O_FILES := $(filter %_reloc.o,$(SPEC_O_FILES))

# Automatic dependency files
# (Only asm_processor dependencies and reloc dependencies are handled for now)
DEP_FILES := $(O_FILES:.o=.d) $(O_FILES:.o=.asmproc.d) $(RELOC_O_FILES:.o=.d) $(BUILD_DIR)/spec.d
...
$(ELF): $(TEXTURE_FILES_OUT) $(ASSET_FILES_OUT) $(O_FILES) $(RELOC_O_FILES) $(LDSCRIPT) $(BUILD_DIR)/linker_scripts/makerom.ld $(BUILD_DIR)/undefined_syms.txt \
...
setup: venv
	...
	$(PYTHON) tools/extract_assets.py $(EXTRACTED_DIR)/baserom $(EXTRACTED_DIR)/assets -v $(VERSION)
...
$(BUILD_DIR)/data/%.o: data/%.s
	$(CPP) $(CPPFLAGS) -MD -MF $(@:.o=.d) -MT $@ -Iinclude $< | iconv --from UTF-8 --to EUC-JP | $(AS) $(ASFLAGS) -o $@
...
$(BUILD_DIR)/src/overlays/%_reloc.o: $(BUILD_DIR)/spec
	$(FADO) -M $(@:.o=.d) -n $(notdir $*) -o $(@:.o=.s) $$($(SPEC_LS_SEG) $(BUILD_DIR)/spec $(notdir $*) | grep -v _reloc.o)
	$(POSTPROCESS_OBJ) $(@:.o=.s)
	$(AS) $(ASFLAGS) $(@:.o=.s) -o $@
