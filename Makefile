...
ASSETS_ORIGINAL_DIRS := assets/original/handpushplayground/exported/handpushplayground

# source files
C_FILES       := $(filter-out %.inc.c,$(foreach dir,$(SRC_DIRS) $(ASSET_BIN_DIRS) $(ASSETS_ORIGINAL_DIRS),$(wildcard $(dir)/*.c)))
...
$(shell mkdir -p build/baserom build/assets/text $(foreach dir,$(SRC_DIRS) $(UNDECOMPILED_DATA_DIRS) $(ASSET_BIN_DIRS) $(ASSETS_ORIGINAL_DIRS),build/$(dir)))
