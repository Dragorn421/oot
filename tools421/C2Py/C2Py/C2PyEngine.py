...
ctypes.Structure = ctypes.BigEndianStructure
...
STRUCTS_REGEX = re.compile(
    r"(typedef\s+)?(?P<PACKED>\w+)?\s*(?P<TYPE>union|struct|enum)\s+(?P<NAME2>\w*)\s*\{(?P<FIELDS>.*?(?:\{.*?\}.*?)*?)\}\s*(?P<NAME>[^;\s]*)\s*;",
    re.S)
...
class C2PyConverter(object):
    ...
    def _extract_instance(self, struct_data, object_type, struct_type_name):
        ...
        #print("_extract_instance() struct_data =", repr(struct_data))
        if all(c not in struct_data for c in "()"):
            ...
            #print("_extract_instance() new_struct_data =", repr(new_struct_data))
    ...
    def _get_type(self, field_type):
        ...
        if POINTER_MARK in field_type:
            # nope, for endianness error better as raw
