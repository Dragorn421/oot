...
void KaleidoScope_DrawQuestStatus(PlayState* play, GraphicsContext* gfxCtx) {
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            if ((pauseCtx->state != 6) || ((pauseCtx->stickAdjX == 0) && (pauseCtx->stickAdjY == 0))) {
                ...
                if (pauseCtx->cursorPoint[PAUSE_QUEST] != 0x18) {
                    ...
                    if ((GET_INVENTORY_QUESTITEMS & 0xF0000000) != 0) {
                        cursorItem = ITEM_HEART_CONTAINER;
                    } else {
                        cursorItem = PAUSE_ITEM_NONE;
                    }
                    osSyncPrintf("888 ccc=%d (%d,  %d,  %x)\n", cursorItem, pauseCtx->cursorPoint[PAUSE_QUEST],
                                 ITEM_HEART_CONTAINER, GET_INVENTORY_QUESTITEMS & 0xF0000000);
                }
    ...
    if ((GET_INVENTORY_QUESTITEMS >> QUEST_HEART_PIECE_COUNT) != 0) {
        gDPPipeSync(POLY_OPA_DISP++);
        ...
        POLY_OPA_DISP = KaleidoScope_QuadTextureIA8(
            POLY_OPA_DISP,
            gItemIcons[0x79 + (((GET_INVENTORY_QUESTITEMS & 0xF0000000) & 0xF0000000) >> QUEST_HEART_PIECE_COUNT)], 48,
            48, 0);
    }
    ...
    if (CHECK_QUEST_ITEM(QUEST_SKULL_TOKEN)) {
        ...
        sp208[0] = sp208[1] = 0;
        sp208[2] = GET_INVENTORY_GSTOKENS;
        ...
        for (phi_s3 = 0, sp218 = 0, sp21A = 0; phi_s3 < 2; phi_s3++) {
            ...
            } else if (GET_INVENTORY_GSTOKENS == 100) {
