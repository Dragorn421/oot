...
void KaleidoScope_DrawDungeonMap(PlayState* play, GraphicsContext* gfxCtx) {
                ...
                if (pauseCtx->cursorX[PAUSE_MAP] != 0) {
                    ...
                    if (!CHECK_DUNGEON_ITEM(DUNGEON_KEY_BOSS, GET_MAPINDEX)) {
                        ...
                        if (!CHECK_DUNGEON_ITEM(DUNGEON_COMPASS, GET_MAPINDEX)) {
                            pauseCtx->cursorPoint[PAUSE_MAP]++;
                            if (!CHECK_DUNGEON_ITEM(DUNGEON_MAP, GET_MAPINDEX)) {
                                KaleidoScope_MoveCursorToSpecialPos(play, PAUSE_CURSOR_PAGE_RIGHT);
                            }
                        }
                    }
                ...
                if (pauseCtx->stickAdjY > 30) {
                    if (pauseCtx->cursorPoint[PAUSE_MAP] != 0) {
                        for (i = pauseCtx->cursorPoint[PAUSE_MAP] - 1; i >= 0; i--) {
                            if (CHECK_DUNGEON_ITEM(i, GET_MAPINDEX)) {
    ...
    gDPLoadTextureBlock(POLY_OPA_DISP++, dungeonTitleTexs[GET_MAPINDEX], G_IM_FMT_IA, G_IM_SIZ_8b, 96, 16, 0,
    ...
    if (GET_GS_FLAGS(GET_MAPINDEX) == gAreaGsFlags[GET_MAPINDEX]) {
...
void KaleidoScope_DrawWorldMap(PlayState* play, GraphicsContext* gfxCtx) {
    ...
    if ((pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) && (pauseCtx->pageIndex == PAUSE_MAP)) {
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            pauseCtx->cursorItem[PAUSE_MAP] = GET_WORLDMAPAREA + 0x18;
    ...
    if (ZREG(38) == 0) {
        ...
        for (k = 0; k < 15; k += 8) {
            ...
            for (j = i = 0; i < 8; i++, j += 4) {
                if (!(GET_WORLDMAPAREADATA & gBitFlags[cloudFlagNums[k + i]])) {
    ...
    if (GET_WORLDMAPAREA < 22) {
        ...
        pauseCtx->mapPageVtx[172].v.ob[1] = pauseCtx->mapPageVtx[173].v.ob[1] =
            areaBoxPosY[(GET_WORLDMAPAREA_VOID0)] + pauseCtx->offsetY;
        ...
        gDPLoadTextureBlock_4b(POLY_OPA_DISP++, areaBoxTexs[((void)0, (GET_WORLDMAPAREA))], G_IM_FMT_IA,
                               areaBoxWidths[((void)0, (GET_WORLDMAPAREA))],
                               areaBoxHeights[((void)0, (GET_WORLDMAPAREA))], 0, G_TX_WRAP | G_TX_NOMIRROR,
                               G_TX_WRAP | G_TX_NOMIRROR, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
