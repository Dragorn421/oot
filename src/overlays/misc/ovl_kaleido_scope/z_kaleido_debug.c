...
void KaleidoScope_DrawDebugEditor(PlayState* play) {
    ...
    // Health capacity
    spD8[2] = 0;
    spD8[3] = GET_HEALTH_CAPACITY / 0x10;
    while (spD8[3] >= 10) {
        spD8[2]++;
        spD8[3] -= 10;
    }
    ...
    for (slot = 0, i = 0, y = 35; i < 4; i++, y += 15) {
        for (j = 0, x = 78; j < 6; j++, slot++, x += 26) {
            ...
            if ((slot <= SLOT_BOW) || (slot == SLOT_SLINGSHOT) || (slot == SLOT_BOMBCHU) || (slot == SLOT_BEAN)) {
                ...
                spD8[3] = GET_INVENTORY_ITEMS[slot];
            } else {
                spD8[3] = GET_INVENTORY_ITEMS[slot];
            }
    ...
    for (spD8[1] = 78, i = 0; i < 17; i++) {
        ...
        if ((spD8[3] = GET_INVENTORY_DUNGEON_KEYS[i]) >= 0) {
            while (spD8[3] >= 10) {
                spD8[2]++;
                spD8[3] -= 10;
            }
    ...
    for (spD8[1] = 78, i = 0; i < 12; i++, spD8[1] += 12) {
        spD8[2] = GET_INVENTORY_DUNGEON_ITEMS[i] & gEquipMasks[0];
    ...
    // GS Tokens
    spD8[3] = GET_INVENTORY_GS_TOKENS;
    spD8[1] = 0;
    spD8[2] = 0;
    while (spD8[3] >= 100) {
        spD8[1]++;
    ...
    KaleidoScope_DrawDigit(
        play, ((GET_INVENTORY_QUEST_ITEMS & 0xF0000000) & 0xF0000000) >> QUEST_HEART_PIECE_COUNT, 210,
        185);
    ...
    switch (curSection) {
        ...
        case 1:
            if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                ...
                if (GET_HEALTH_CAPACITY < 0x30) {
                    SET_HEALTH_CAPACITY(0x30)
                }
            } else if (CHECK_BTN_ALL(input->press.button, BTN_CDOWN) ||
                       CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                GET_HEALTH_CAPACITY += 0x10;
                if (GET_HEALTH_CAPACITY >= 0x140) {
                    SET_HEALTH_CAPACITY(0x140)
                }
            }
            break;
        ...
        case 0x5C:
            if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                if ((((GET_INVENTORY_QUEST_ITEMS & 0xF0000000) & 0xF0000000) >>
                     QUEST_HEART_PIECE_COUNT) != 0) {
                    GET_INVENTORY_QUEST_ITEMS -= (1 << QUEST_HEART_PIECE_COUNT);
                }
            } else if (CHECK_BTN_ALL(input->press.button, BTN_CDOWN) ||
                       CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                if ((GET_INVENTORY_QUEST_ITEMS & 0xF0000000) <= (4 << QUEST_HEART_PIECE_COUNT)) {
                    GET_INVENTORY_QUEST_ITEMS += (1 << QUEST_HEART_PIECE_COUNT);
                }
            }
            break;
        ...
        default:
            if (curSection < 0x1B) {
                ...
                if ((i <= SLOT_BOW) || (i == SLOT_SLINGSHOT) || (i == SLOT_BOMBCHU) || (i == SLOT_BEAN)) {
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP)) {
                        ...
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_OCARINA_FAIRY;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_OCARINA_FAIRY) &&
                                   (GET_INVENTORY_ITEMS[i] < ITEM_OCARINA_TIME)) {
                            GET_INVENTORY_ITEMS[i]++;
                        }
                    } else if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_OCARINA_TIME;
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP)) {
                        ...
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_HOOKSHOT;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_HOOKSHOT) &&
                                   (GET_INVENTORY_ITEMS[i] < ITEM_LONGSHOT)) {
                            GET_INVENTORY_ITEMS[i]++;
                        }
                    } else if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP)) {
                        ...
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_POCKET_EGG;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_POCKET_EGG) &&
                                   (GET_INVENTORY_ITEMS[i] < ITEM_CLAIM_CHECK)) {
                            GET_INVENTORY_ITEMS[i]++;
                        }
                    } else if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP)) {
                        ...
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_WEIRD_EGG;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_WEIRD_EGG) &&
                                   (GET_INVENTORY_ITEMS[i] < ITEM_SOLD_OUT)) {
                            GET_INVENTORY_ITEMS[i]++;
                        }
                    } else if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP)) {
                        ...
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_BOTTLE;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_BOTTLE) &&
                                   (GET_INVENTORY_ITEMS[i] <= ITEM_MILK_HALF)) {
                            GET_INVENTORY_ITEMS[i]++;
                        }
                    } else if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                            GET_INVENTORY_ITEMS[i] = ITEM_POE;
                        } else if ((GET_INVENTORY_ITEMS[i] >= ITEM_POTION_RED) &&
                                   (GET_INVENTORY_ITEMS[i] <= ITEM_POE)) {
                            GET_INVENTORY_ITEMS[i]--;
                        }
                    }
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT) ||
                        ...
                        if (i == SLOT_TRADE_ADULT) {
                            if (GET_INVENTORY_ITEMS[i] == ITEM_NONE) {
                                GET_INVENTORY_ITEMS[i] = ITEM_BEAN;
                            } else {
                ...
                if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                    i = curSection - 0x1B;
                    GET_INVENTORY_DUNGEON_KEYS[i]--;
                    if (GET_INVENTORY_DUNGEON_KEYS[i] < 0) {
                        GET_INVENTORY_DUNGEON_KEYS[i] = -1;
                    }
                } else if (CHECK_BTN_ALL(input->press.button, BTN_CDOWN) ||
                           CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                    i = curSection - 0x1B;
                    if (GET_INVENTORY_DUNGEON_KEYS[i] < 0) {
                        ...
                        GET_INVENTORY_DUNGEON_KEYS[i]++;
                        if (GET_INVENTORY_DUNGEON_KEYS[i] >= 9) {
                            GET_INVENTORY_DUNGEON_KEYS[i] = 9;
                        }
                    }
                }
            } else {
                if (curSection < 0x38) {
                    ...
                    i = curSection - 0x38;
                    if (CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                        GET_INVENTORY_DUNGEON_ITEMS[i] ^= 4;
                    }
                    if (CHECK_BTN_ALL(input->press.button, BTN_CDOWN)) {
                        GET_INVENTORY_DUNGEON_ITEMS[i] ^= 2;
                    }
                    if (CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        GET_INVENTORY_DUNGEON_ITEMS[i] ^= 1;
                    }
                    ...
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                        GET_INVENTORY_GS_TOKENS++;
                               ...
                               CHECK_BTN_ALL(input->press.button, BTN_CRIGHT)) {
                        GET_INVENTORY_GS_TOKENS--;
                        if (GET_INVENTORY_GS_TOKENS <= 0) {
                            SET_INVENTORY_GS_TOKENS(0)
                        }
                    }
                } else if (curSection < 0x5C) {
                    i = curSection - 0x44;
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
                        GET_INVENTORY_QUEST_ITEMS ^= gBitFlags[i];
                    }
                }
            }
            break;
