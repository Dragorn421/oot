...
static ColliderSpheresElementSrc sBodyElementsInit[6] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static ColliderSpheresSrc sBodyJntSphInit = {
    {
        COL_MATERIAL_HIT0,
        AT_ON | AT_TYPE_ENEMY,
...
static ColliderTrisElementSrc sHardElementsInit[3] = {
    {
        {
            ELEM_MATERIAL_UNK2,
            {
...
static ColliderTrisSrc sHardTrisInit = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
...
static ColliderQuadSrc sAttackQuadInit = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
void EnDodongo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_InitSpheres(globalCtx, &this->colliderBody);
    Collider_LoadQuad(globalCtx, &this->colliderAT, &this->actor, &sAttackQuadInit);
    Collider_LoadTris(globalCtx, &this->colliderHard, &this->actor, &sHardTrisInit, this->trisElements);
    Collider_LoadSpheres(globalCtx, &this->colliderBody, &this->actor, &sBodyJntSphInit, this->sphElements);
...
void EnDodongo_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->colliderBody);
        ...
        if (this->timer != 0) {
            ...
            if (this->timer == 10) {
                for (i = 10; i >= 0; i--) {
                    ...
                    pos.x = this->sphElements[0].shape.world.center.x + deathFireVel.x;
                    pos.y = this->sphElements[0].shape.world.center.y + deathFireVel.y;
                    pos.z = this->sphElements[0].shape.world.center.z + deathFireVel.z;
...
void EnDodongo_SweepTail(EnDodongo* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_FrameUpdateMatrix(&this->skelAnime)) {
        if ((this->timer != 0) || (ABS(yawDiff1) < 0x4000)) {
            this->sphElements[2].base.atElemFlags = 0;
            this->sphElements[1].base.atElemFlags = 0;
            ...
            this->colliderBody.base.atFlags = this->sphElements[1].base.atElemFlags =
                this->sphElements[2].base.atElemFlags = 0x11;
            this->sphElements[1].base.atDmgInfo.dmgFlags = this->sphElements[2].base.atDmgInfo.dmgFlags = 0xFFCFFFFF;
        ...
        tailPos.x = this->sphElements[1].shape.world.center.x;
        tailPos.y = this->sphElements[1].shape.world.center.y;
        tailPos.z = this->sphElements[1].shape.world.center.z;
        func_80033260(globalCtx, &this->actor, &tailPos, 5.0f, 2, 2.0f, 100, 15, 0);
        tailPos.x = this->sphElements[2].shape.world.center.x;
        tailPos.y = this->sphElements[2].shape.world.center.y;
        tailPos.z = this->sphElements[2].shape.world.center.z;
        func_80033260(globalCtx, &this->actor, &tailPos, 5.0f, 2, 2.0f, 100, 15, 0);
        ...
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderBody.base);
    }
}
...
void EnDodongo_CollisionCheck(EnDodongo* this, GlobalContext* globalCtx) {
    if (this->colliderHard.base.acFlags & 0x80) {
        ...
        if (this->actor.collideData.damageEffect != 0xE) {
            this->damageEffect = this->actor.collideData.damageEffect;
            if ((this->actor.collideData.damageEffect == 1) || (this->actor.collideData.damageEffect == 0xF)) {
...
void EnDodongo_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderBody.base);
    if (this->actionState != DODONGO_DEATH) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderHard.base);
    }
    if (this->actionState > DODONGO_DEATH) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderBody.base);
    }
    ...
    if (this->actionState == DODONGO_BREATHE_FIRE) {
        if ((29.0f < this->skelAnime.animCurrentFrame) && (this->skelAnime.animCurrentFrame < 43.0f)) {
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderAT.base);
        }
    }
...
void EnDodongo_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    func__800628A4_Type0(limbIndex, &this->colliderBody);
