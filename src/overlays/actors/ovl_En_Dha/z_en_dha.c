...
void EnDha_UpdateHealth(EnDha* this, GlobalContext* globalCtx) {
    if (!((this->unk_1C0 >= 8) || !(this->collider.base.acFlags & 2))) {
        ...
        if (this->actor.collideData.damageEffect == 0 || this->actor.collideData.damageEffect == 6) {
            return;
        } else {
...
void EnDha_Update(Actor* thisx, GlobalContext* globalCtx) {
    EnDha* this = THIS;
    ColliderContext* colliderCtx;

    colliderCtx = &globalCtx->colliderCtx;
...
void EnDha_OverridePostDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    switch (limbIndex) {
        case 1:
            func__800628A4_Type0(2, &this->collider);
            func__800628A4_Type0(3, &this->collider);
            break;

        case 2:
            func__800628A4_Type0(4, &this->collider);
            func__800628A4_Type0(5, &this->collider);
            Matrix_MultVec3f(&D_809ED770, &this->unk_1F4);
            break;

        case 3:
            func__800628A4_Type0(1, &this->collider);
