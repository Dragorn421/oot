...
static ColliderCylinderSrc sPotColliderInit = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
...
static ColliderCylinderSrc sFlamesColliderInit = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
void BgHakaTubo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->potCollider, &this->dyna.actor, &sPotColliderInit);
    Collider_InitCylinder(globalCtx, &this->flamesCollider);
    Collider_LoadCylinder(globalCtx, &this->flamesCollider, &this->dyna.actor, &sFlamesColliderInit);
...
void BgHakaTubo_Idle(BgHakaTubo* this, GlobalContext* globalCtx) {
    ...
    if (this->potCollider.base.acFlags & 2) {
        ...
        Collider_UpdateCylinderShape(&this->dyna.actor, &this->flamesCollider);
        Collider_UpdateCylinderShape(&this->dyna.actor, &this->potCollider);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->potCollider.base);
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->flamesCollider.base);
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->flamesCollider.base);
    }
}
