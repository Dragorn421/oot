...
void BgHidanKowarerukabe_InitColliderSphere(BgHidanKowarerukabe* this, GlobalContext* globalCtx) {
    ...
    this->collider.elements[0].shape.model.radius = sphereRadii[this->dyna.actor.params & 0xFF];
    this->collider.elements[0].shape.model.center.y = sphereYPositions[this->dyna.actor.params & 0xFF];
}
