...
static CollideDataInitAlt D_80A1FB94 = { 8, 2, 25, 25, 0xFF };
...
s32 EnFw_CheckCollider(EnFw* this, GlobalContext* globalCtx) {
    ...
    if (this->collider.base.acFlags & 2) {
        ...
        if (body->acHitElem->atDmgInfo.dmgFlags & 0x80) {
        ...
        if (Actor_ApplyDamage(&this->actor) <= 0) {
            if (this->actor.parent->collideData.health <= 8) {
                func_80032C7C(globalCtx, &this->actor);
                this->actor.parent->collideData.health = 0;
            } else {
                this->actor.parent->collideData.health -= 8;
            }
...
void EnFw_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadSpheres(globalCtx, &this->collider, &this->actor, &sJntSphInit, this->sphs);
    func_80061EFC(&this->actor.collideData, CollisionBtlTbl_Get(0x10), &D_80A1FB94);
