...
static ColliderSpheresElementSrc sJntSphItemsInit[12] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static CollideDataInitAlt sColChkInit = { 24, 2, 25, 25, 0xFF };
...
s32 EnFd_SpawnCore(EnFd* this, GlobalContext* globalCtx) {
    ...
    this->actor.child->collideData.health = this->actor.collideData.health % 8;

    if (this->actor.child->collideData.health == 0) {
        this->actor.child->collideData.health = 8;
    }
...
s32 EnFd_ColliderCheck(EnFd* this, GlobalContext* globalCtx) {
    ...
    if (this->collider.base.acFlags & 2 || EnFd_CheckHammer(this, globalCtx)) {
        ...
        body = &this->collider.elements[0].base;
        if (body->acHitElem != NULL && (body->acHitElem->atDmgInfo.dmgFlags & 0x80)) {
            return false;
        }
...
void EnFd_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadSpheres(globalCtx, &this->collider, &this->actor, &sJntSphInit, this->colSphs);
    func_80061EFC(&this->actor.collideData, CollisionBtlTbl_Get(0xF), &sColChkInit);
    this->actor.flags &= ~1;
...
void EnFd_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP) {
    ...
    func__800628A4_Type0(limbIndex, &this->collider);
}
...
void EnFd_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actionFunc != EnFd_Reappear && !(this->fadeAlpha < 0.9f)) {
        ...
        clampedHealth = CLAMP(thisx->collideData.health - 1, 0, 23);
