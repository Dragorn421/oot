...
static ColliderSpheresElementSrc sJntSphItemsInit[2] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static CollideDataInitAlt sCollideDataInit2 = {
...
u32 func_809CBCBC(EnBubble* this) {
    ColliderElement* body = &this->colliderSphere.elements->base;

    body->atDmgInfo.dmgFlags = 0x8;
    body->atDmgInfo.hitSpecialEffect = 0;
    body->atDmgInfo.damage = 4;
    body->atElemFlags = 1;
...
void EnBubble_DamagePlayer(EnBubble* this, GlobalContext* globalCtx) {
    s32 damage = -this->colliderSphere.elements->base.atDmgInfo.damage;
...
void EnBubble_Fly(EnBubble* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderSphere.elements[1].base.acElemFlags & 0x2) {
...
u32 EnBubble_DetectPop(EnBubble* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderSphere.base.ocFlags2 & 0x1) {
        this->colliderSphere.base.ocFlags2 &= ~0x1;
...
void func_809CC774(EnBubble* this) {
    ColliderSpheresElementShape* dim;
    Vec3f src;
    Vec3f dest;

    dim = &this->colliderSphere.elements[0].shape;
    src.x = dim->model.center.x;
    src.y = dim->model.center.y;
    src.z = dim->model.center.z;

    Matrix_MultVec3f(&src, &dest);
    dim->world.center.x = dest.x;
    dim->world.center.y = dest.y;
    dim->world.center.z = dest.z;
    dim->world.radius = dim->model.radius * (1.0f + this->expansionWidth);
    this->colliderSphere.elements[1].shape = *dim;
}
...
void EnBubble_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadSpheres(globalCtx, &this->colliderSphere, &this->actor, &sJntSphInit, this->colliderSphereItems);
    func_80061EFC(&this->actor.collideData, CollisionBtlTbl_Get(9), &sCollideDataInit2);
...
void EnBubble_Wait(EnBubble* this, GlobalContext* globalCtx) {
    if (EnBubble_DetectPop(this, globalCtx)) {
        ...
        this->actor.shape.unk_08 = ((this->expansionHeight + 1.0f) * 16.0f);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderSphere.base);
