...
static ColliderTrisElementSrc sTrimItemInit[3] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
void BgBombwall_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Flags_GetSwitch(globalCtx, this->dyna.actor.params & 0x3F)) {
        ...
        for (i = 0; i <= 2; i++) {
            for (j = 0; j <= 2; j++) {
                sp80.x = sTrisInit.elements[i].vertices[j].x;
                sp80.y = sTrisInit.elements[i].vertices[j].y;
                sp80.z = sTrisInit.elements[i].vertices[j].z + 2.0f;
