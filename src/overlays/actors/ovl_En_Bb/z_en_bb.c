...
static ActorDamageChart sActorDamageChartBlueGreen = {
...
static ActorDamageChart sActorDamageChartRed = {
...
static ActorDamageChart sActorDamageChartWhite = {
...
static ColliderSpheresElementSrc sJntSphElementInit[1] = {
    {
...
void EnBb_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    SkelAnime_Init(globalCtx, &this->skelAnime, &D_06001A30, &D_06000444, this->limbDrawTbl, this->transitionDrawTbl,
                   16);
    this->unk_254 = 0;
    thisx->collideData.health = 4;
    Collider_InitSpheres(globalCtx, &this->collider);
    Collider_LoadSpheres(globalCtx, &this->collider, thisx, &sJntSphInit, this->elements);
    ...
    if (thisx->params & 0xFF00) {
        ...
        this->collider.elements[0].base.atElemFlags = 9;
        this->collider.elements[0].base.atDmgInfo.dmgFlags = 0xFFCFFFFF;
        ...
        switch (thisx->params) {
            case ENBB_BLUE:
                ...
                thisx->collideData.damageChart = &sActorDamageChartBlueGreen;
            ...
            case ENBB_RED:
                ...
                thisx->collideData.damageChart = &sActorDamageChartRed;
                this->flameEnvColor.r = 255;
                this->collider.elements[0].base.atDmgInfo.hitSpecialEffect = 1;
                EnBb_SetupRed(globalCtx, this);
                break;
            case ENBB_WHITE:
                thisx->naviEnemyId = 0x1D;
                thisx->collideData.damageChart = &sActorDamageChartWhite;
            ...
            case ENBB_GREEN_BIG:
                ...
                this->collider.elements[0].shape.model.radius = 0x16;
    ...
    this->collider.elements[0].shape.world.radius =
        this->collider.elements[0].shape.model.radius * this->collider.elements[0].shape.unkRadiusScale;
}
...
void EnBb_SetupRed(GlobalContext* globalCtx, EnBb* this) {
    ...
    if (this->action == BB_DOWN) {
        ...
        this->actor.collideData.health = 4;
        this->timer = 0;
...
void EnBb_CollisionCheck(EnBb* this, GlobalContext* globalCtx) {
    ...
    if (this->collider.base.acFlags & 2) {
        this->collider.base.acFlags &= ~2;
        this->dmgEffect = this->actor.collideData.damageEffect;
        func_80035650(&this->actor, &this->collider.elements[0].base, 0);
        switch (this->dmgEffect) {
            case 7:
...
void EnBb_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (this->actor.collideData.damageEffect != 0xD) {
        ...
        this->collider.elements->shape.world.center.y =
