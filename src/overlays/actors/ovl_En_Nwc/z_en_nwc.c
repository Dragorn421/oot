...
static ColliderSpheresElementSrc sJntSphElementInit = {
    {
        ELEM_MATERIAL_UNK1,
        {
            0x00000000,
...
static ColliderSpheresSrcAlt sJntSphInit = {
    {
        COL_MATERIAL_HIT3,
        AT_NONE,
...
void EnNwc_UpdateChicks(EnNwc* this, GlobalContext* globalCtx) {
    ...
    ColliderSpheresElement* element = this->collider.elements;
    Vec3f prevChickPos;
    s32 i;
    f32 test;
    ...
    for (i = 0; i < this->count; i++, prevChickPos = chick->pos, chick++, element++) {
        ...
        element->shape.world.center.x = chick->pos.x;
        element->shape.world.center.y = chick->pos.y;
        element->shape.world.center.z = chick->pos.z;
...
void EnNwc_Init(Actor* thisx, GlobalContext* globalCtx) {
    s32 pad;
    EnNwc* this = THIS;
    ColliderSpheresElementSrc elementInits[16];
    ColliderSpheresElementSrc* element;
    EnNwcChick* chick;
    s32 i;
    ...
    Collider_LoadSpheresAltMalloc(globalCtx, &this->collider, &this->actor, &sJntSphInit);
    this->count = 16;
    chick = this->chicks;
    for (i = 0; i < this->count; i++, chick++) {
...
void EnNwc_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheresMalloc(globalCtx, &this->collider);
}
