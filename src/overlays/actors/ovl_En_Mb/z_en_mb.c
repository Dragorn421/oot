...
ColliderCylinderSrc sCylinderInit_EnMb = {
    {
        COL_MATERIAL_HIT0,
        AT_NONE,
...
ColliderTrisElementSrc sTrisItemsInit_EnMb[2] = {
    {
        {
            ELEM_MATERIAL_UNK2,
            {
...
ColliderTrisSrc sTrisInit_EnMb = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
...
ColliderQuadSrc sQuadInit_EnMb = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
static ActorDamageChart sBigMoblinActorDamageChart = {
...
void EnMb_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->collider1, &this->actor, &sCylinderInit_EnMb);
    Collider_InitTris(globalCtx, &this->collider3);
    Collider_LoadTris(globalCtx, &this->collider3, &this->actor, &sTrisInit_EnMb, this->collider3Items);
    Collider_InitQuad(globalCtx, &this->collider2);
    Collider_LoadQuad(globalCtx, &this->collider2, &this->actor, &sQuadInit_EnMb);

    switch (this->actor.params) {
        case -1:
            SkelAnime_InitFlex(globalCtx, &this->skelAnime, &D_06008F38, &D_060028E0, this->limbDrawTable,
                               this->transitionDrawTable, 28);
            this->actor.collideData.health = 2;
            this->actor.collideData.mass = 0xFE;
        ...
        case 0:
            SkelAnime_InitFlex(globalCtx, &this->skelAnime, &D_06014190, &D_0600EBE4, this->limbDrawTable,
                               this->transitionDrawTable, 28);
            this->actor.collideData.health = 6;
            this->actor.collideData.mass = 0xFF;
            this->actor.collideData.damageChart = &sBigMoblinActorDamageChart;
            Actor_SetScale(&this->actor, 0.02f);
            this->collider1.shape.height = 170;
            this->collider1.shape.radius = 45;
            this->actor.uncullZoneForward = 4000.0f;
            this->actor.uncullZoneScale = 800.0f;
            this->actor.uncullZoneDownward = 1800.0f;
            this->unk_364 = 710.0f;
            this->collider2.elem.atDmgInfo.dmgFlags = 0x20000000;
        ...
        default:
            ...
            this->actor.params = 1;
            this->waypoint = 0;
            this->actor.collideData.health = 1;
            this->actor.collideData.mass = 0xFE;
            this->unk_360 = 350.0f;
            this->unk_364 = 1750.0f;
            this->actor.flags &= ~1;
...
void func_80AA6F04(EnMb* this) {
    ...
    this->unk_320 = 1;
    this->actor.flags &= ~1;
    this->collider1.shape.height = 80;
    this->collider1.shape.radius = 95;
    this->unk_32A = 30;
    this->actor.speedXZ = 0.0f;
    Audio_PlayActorSound2(&this->actor, NA_SE_EN_MORIBLIN_DEAD);
...
void func_80AA702C(EnMb* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.dmgEffectTimer == 0) {
        ...
        } else if (this->actor.collideData.health == 0) {
            func_80AA8FC8(this);
        } else {
            func_80AA8E88(this);
        }
        ...
        if (this->unk_32A != 0) {
            ...
            if (this->unk_32E == 0) {
                ...
                if ((ABS(this->actor.yDistFromLink) <= 20.0f) && (func_80AA652C(this, globalCtx) != 0) &&
                    ...
                    SkelAnime_ChangeAnim(&this->skelAnime, &D_06002C10, -1.0f, SkelAnime_GetFrameCount(&D_06002C10),
                                         0.0f, 2, 0.0f);
                    this->actor.speedXZ = 0.0f;
                    Audio_PlayActorSound2(&this->actor, NA_SE_EN_MORIBLIN_SPEAR_NORM);
                }
    ...
    if ((this->unk_32E == 0) &&
        ...
        if (this->unk_32A == 0) {
            if (Math_Rand_ZeroOne() > 0.7f) {
                this->unk_32A = Math_Rand_S16Offset(50, 70);
                this->soundTimer = Math_Rand_S16Offset(26, 40) & 0xFFFF;
            } else {
                func_80AA6830(this);
            }
        }
...
void func_80AA92B8(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_MultVec3f(&D_80AA9D90, &this->collider2.shape.corners.cornerC);
    Matrix_MultVec3f(&D_80AA9D9C, &this->collider2.shape.corners.cornerD);
    Matrix_MultVec3f(&D_80AA9DA8, &this->collider2.shape.corners.cornerB);
    Matrix_MultVec3f(&D_80AA9DB4, &this->collider2.shape.corners.cornerA);
    func_80062734(&this->collider2, &this->collider2.shape.corners.cornerD, &this->collider2.shape.corners.cornerC,
                  &this->collider2.shape.corners.cornerA, &this->collider2.shape.corners.cornerB);
}
...
void func_80AA9440(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_MultVec3f(&D_80AA9DC0, &this->collider2.shape.corners.cornerC);
    Matrix_MultVec3f(&D_80AA9DCC, &this->collider2.shape.corners.cornerD);
    Matrix_MultVec3f(&D_80AA9DD8, &this->collider2.shape.corners.cornerB);
    Matrix_MultVec3f(&D_80AA9DE4, &this->collider2.shape.corners.cornerA);
    func_80062734(&this->collider2, &this->collider2.shape.corners.cornerD, &this->collider2.shape.corners.cornerC,
                  &this->collider2.shape.corners.cornerA, &this->collider2.shape.corners.cornerB);
}
...
void func_80AA94D8(EnMb* this, GlobalContext* globalCtx) {
    ...
    if ((this->collider1.base.acFlags & 2) && (this->unk_320 >= 5)) {
        this->collider1.base.acFlags &= ~2;
        if ((this->actor.collideData.damageEffect != 0) && (this->actor.collideData.damageEffect != 5)) {
            if ((player->stateFlags2 & 0x80) && &this->actor == player->actor.parent) {
                player->stateFlags2 &= ~0x80;
                player->actor.parent = NULL;
                player->unk_850 = 200;
            ...
            this->unk_188 = this->actor.collideData.damageEffect;
            this->attackParams = 0;
            func_80035650(&this->actor, &this->collider1.elem, 0);
            if ((this->actor.collideData.damageEffect == 1) || (this->actor.collideData.damageEffect == 6)) {
                if (this->unk_320 != 5) {
...
void EnMb_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->collideData.damageEffect != 5) {
        this->actionFunc(this, globalCtx);
        Actor_MoveForward(thisx);
        func_8002E4B4(globalCtx, thisx, 40.0f, 40.0f, 70.0f, 0x1D);
        Actor_SetHeight(thisx, thisx->scale.x * 4500.0f);
        Collider_UpdateCylinderShape(thisx, &this->collider1);
        if (thisx->collideData.health <= 0) {
            this->collider1.shape.pos.x += (Math_Sins(thisx->shape.rot.y) * (-4400.0f * thisx->scale.y));
            this->collider1.shape.pos.z += (Math_Coss(thisx->shape.rot.y) * (-4400.0f * thisx->scale.y));
        }
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->collider1.base);
        if (this->unk_320 >= 5) {
            if ((thisx->params == 0) || (this->unk_320 != 10)) {
                Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->collider1.base);
            }
        }
        if (this->unk_320 >= 6) {
            Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->collider3.base);
        }
        if (this->attackParams > 0) {
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->collider2.base);
        }
    }
}
