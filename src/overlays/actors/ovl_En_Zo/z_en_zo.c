...
static CollideDataInitAlt sColChkInit = { 0, 0, 0, 0, 0xFF };
...
void EnZo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80061EFC(&this->actor.collideData, NULL, &sColChkInit);
    ...
    this->actor.unk_1F = 6;
    this->dialogRadius = this->collider.shape.radius + 30.0f;
    this->unk_64C = 1;
    this->canSpeak = false;
    this->unk_194.unk_00 = 0;
    func_8002E4B4(globalCtx, &this->actor, this->collider.shape.height * 0.5f, this->collider.shape.radius, 0.0f, 5);
...
void EnZo_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_8002E4B4(globalCtx, thisx, this->collider.shape.radius, this->collider.shape.height * 0.25f, 0.0f, 5);
