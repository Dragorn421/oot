...
void func_80A3F908(EnGo* this, PlayState* play) {
    ...
    if (this->actionFunc == EnGo_BiggoronActionFunc || this->actionFunc == EnGo_GoronLinkRolling ||
        ...
        if ((this->actor.params & 0xF0) == 0x90) {
            isUnkCondition =
                func_80A3ED24(play, &this->actor, &this->interactInfo, float1, EnGo_GetTextID, EnGo_UpdateTalkState);
        } else {
            isUnkCondition =
                Npc_UpdateTalking(play, &this->actor, &this->interactInfo.talkState, float1, EnGo_GetTextID, EnGo_UpdateTalkState);
        }
