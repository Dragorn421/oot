...
ColliderCylinderSrc sCylinderInit_EnBx = {
    {
        COL_MATERIAL_HIT6,
        AT_ON | AT_TYPE_ENEMY,
...
ColliderQuadSrc sQuadInit_EnBx = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
void EnBx_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInit_EnBx);
    Collider_InitQuad(globalCtx, &this->colliderQuad);
    Collider_LoadQuad(globalCtx, &this->colliderQuad, &this->actor, &sQuadInit_EnBx);
...
void func_809D1D0C(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Matrix_MultVec3f(&sp5C, &this->colliderQuad.shape.corners.cornerC);
    Matrix_MultVec3f(&sp50, &this->colliderQuad.shape.corners.cornerD);
    func_80062734(&this->colliderQuad, &sp38, &sp44, &this->colliderQuad.shape.corners.cornerD, &this->colliderQuad.shape.corners.cornerC);
}
...
void EnBx_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_UpdateCylinderShape(thisx, &this->collider);
    ...
    if (thisx->params & 0x80) {
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderQuad.base);
    }
}
