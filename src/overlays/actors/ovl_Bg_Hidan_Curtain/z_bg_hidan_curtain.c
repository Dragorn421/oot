...
static CollideDataInit sCcInfoInit = { 1, 0x50, 0x64, 0xFF };
...
void BgHidanCurtain_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->collider.shape.radius = hcParams->radius;
    this->collider.shape.height = hcParams->height;
    Collider_UpdateCylinderShape(&this->actor, &this->collider);
    func_80061ED4(&thisx->collideData, NULL, &sCcInfoInit);
    if (this->type == 0) {
...
void BgHidanCurtain_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if ((globalCtx->cameraPtrs[0]->setting == 0x28) || (globalCtx->cameraPtrs[0]->setting == 0x38)) {
        ...
        if (this->alpha > 50) {
            this->collider.shape.height = hcParams->height * riseProgress;
