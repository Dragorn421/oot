...
static ColliderQuadSrc sQuadInit1 = {
    {
        COL_MATERIAL_METAL,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
static ColliderQuadSrc sQuadInit2 = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
...
void EnVm_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->colliderCylinder, thisx, &sCylinderInit);
    Collider_InitQuad(globalCtx, &this->colliderQuad1);
    Collider_LoadQuad(globalCtx, &this->colliderQuad1, thisx, &sQuadInit1);
    Collider_InitQuad(globalCtx, &this->colliderQuad2);
    Collider_LoadQuad(globalCtx, &this->colliderQuad2, thisx, &sQuadInit2);
    this->beamSightRange = (thisx->params >> 8) * 40.0f;
    thisx->params &= 0xFF;
    thisx->naviEnemyId = 0x39;

    if (thisx->params == BEAMOS_LARGE) {
        thisx->collideData.health = 2;
...
void EnVm_Attack(EnVm* this, GlobalContext* globalCtx) {
    ...
    if (this->beamRot.x < 0xAAA || this->timer == 0) {
        ...
        if (this->unk_260 > 2) {
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderQuad1);
        }
...
void EnVm_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.collideData.health != 0 && this->unk_21C != 2) {
    ...
    Collider_AddOC(globalCtx, colliderCtx, &this->colliderCylinder);

    if (this->actor.dmgEffectTimer == 0 && this->actor.collideData.health != 0) {
        Collider_AddAC(globalCtx, colliderCtx, &this->colliderCylinder);
    }

    Collider_AddAC(globalCtx, colliderCtx, &this->colliderQuad2);
    ...
    if (limbIndex == 2) {
        ...
        if (this->unk_260 >= 3) {
            ...
            if (this->beamScale.z != 0.0f) {
                ...
                Matrix_MultVec3f(&D_80B2EB64, &this->colliderQuad1.shape.corners.cornerB);
                Matrix_MultVec3f(&D_80B2EB70, &this->colliderQuad1.shape.corners.cornerA);
                Matrix_MultVec3f(&sp74, &this->colliderQuad1.shape.corners.cornerC);
                Matrix_MultVec3f(&sp68, &this->colliderQuad1.shape.corners.cornerD);
                func_80062734(&this->colliderQuad1, &this->colliderQuad1.shape.corners.cornerD,
                              &this->colliderQuad1.shape.corners.cornerC, &this->colliderQuad1.shape.corners.cornerA,
                              &this->colliderQuad1.shape.corners.cornerB);
            }
        }
        Matrix_MultVec3f(&D_80B2EB34, &this->colliderQuad2.shape.corners.cornerC);
        Matrix_MultVec3f(&D_80B2EB40, &this->colliderQuad2.shape.corners.cornerD);
        Matrix_MultVec3f(&D_80B2EB4C, &this->colliderQuad2.shape.corners.cornerB);
        Matrix_MultVec3f(&D_80B2EB58, &this->colliderQuad2.shape.corners.cornerA);
        func_80062734(&this->colliderQuad2, &this->colliderQuad2.shape.corners.cornerD,
                      &this->colliderQuad2.shape.corners.cornerC, &this->colliderQuad2.shape.corners.cornerA,
                      &this->colliderQuad2.shape.corners.cornerB);
    }
}
