...
static CollideDataInitAlt sColChkInit = { 0x02, 0x0000, 0x0000, 0x0000, 0xFF };

static ColliderCylinderSrc sCylinderInit2 = {
    {
        COL_MATERIAL_HIT6,
        AT_NONE,
        AC_NONE,
...
void EnSt_InitColliders(EnSt* this, GlobalContext* globalCtx) {
    ColliderCylinderSrc* cylinders[6] = {
    ...
    for (i = 0; i < ARRAY_COUNT(cylinders); i++) {
        ...
        Collider_LoadCylinder(globalCtx, &this->colCylinder[i], &this->actor, cylinders[i]);
    }

    this->colCylinder[0].elem.acDmgInfo.dmgFlags = 0x3F8F9;
    this->colCylinder[1].elem.acDmgInfo.dmgFlags = 0xFFC00706;
    this->colCylinder[2].base.colMaterial = 9;
    this->colCylinder[2].elem.acElemFlags = 0xD;
    this->colCylinder[2].elem.elemMaterial = 2;
    this->colCylinder[2].elem.acDmgInfo.dmgFlags = 0xFFCC0706;

    func_80061EFC(&this->actor.collideData, CollisionBtlTbl_Get(2), &sColChkInit);

    Collider_InitSpheres(globalCtx, &this->colSph);
    Collider_LoadSpheres(globalCtx, &this->colSph, &this->actor, &sJntSphInit, this->colSphItems);
}
...
void EnSt_SetBodyCylinderAC(EnSt* this, GlobalContext* globalCtx) {
    Collider_UpdateCylinderShape(&this->actor, &this->colCylinder[0]);
    Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colCylinder[0].base);
}
...
void EnSt_SetLegsCylinderAC(EnSt* this, GlobalContext* globalCtx) {
    ...
    if (angleTowardsLink < 0x3FFC) {
        Collider_UpdateCylinderShape(&this->actor, &this->colCylinder[2]);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colCylinder[2].base);
    } else {
        Collider_UpdateCylinderShape(&this->actor, &this->colCylinder[1]);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colCylinder[1].base);
    }
}
...
s32 EnSt_SetCylinderOC(EnSt* this, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        Matrix_Pull();
        this->colCylinder[i + 3].shape.pos.x = cylPos.x;
        this->colCylinder[i + 3].shape.pos.y = cylPos.y;
        this->colCylinder[i + 3].shape.pos.z = cylPos.z;
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colCylinder[i + 3].base);
    }
...
void EnSt_UpdateCylinders(EnSt* this, GlobalContext* globalCtx) {
    if ((this->actor.collideData.health != 0) || (this->actionFunc == EnSt_FinishBouncing)) {
...
s32 EnSt_CheckHitLink(EnSt* this, GlobalContext* globalCtx) {
    ...
    for (i = 0, hit = 0; i < 3; i++) {
        if (((this->colCylinder[i + 3].base.ocFlags2 & 1) != 0) == 0) {
            continue;
        }
        this->colCylinder[i + 3].base.ocFlags2 &= ~0x1;
        hit = true;
    }
...
s32 EnSt_CheckHitBackside(EnSt* this, GlobalContext* globalCtx) {
    ...
    this->invulnerableTimer = 8;
    if (this->actor.collideData.damageEffect == 1) {
        if (this->stunTimer == 0) {
...
void EnSt_SetColliderScale(EnSt* this) {
    ...
    radius = this->colSph.elements[0].shape.model.radius;
    radius *= scaleAmount;
    this->colSph.elements[0].shape.model.radius = radius;

    for (i = 0; i < 6; i++) {
        yShift = this->colCylinder[i].shape.yShift;
        radius = this->colCylinder[i].shape.radius;
        height = this->colCylinder[i].shape.height;
        yShift *= scaleAmount;
        radius *= scaleAmount;
        height *= scaleAmount;

        this->colCylinder[i].shape.yShift = yShift;
        this->colCylinder[i].shape.radius = radius;
        this->colCylinder[i].shape.height = height;
    }
...
void EnSt_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->colSph);
}
...
void EnSt_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dListP, Vec3s* rot, void* thisx) {
    ...
    func__800628A4_Type0(limbIndex, &this->colSph);
}
