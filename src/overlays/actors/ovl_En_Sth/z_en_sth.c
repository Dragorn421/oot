...
void EnSth_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params == 0) {
        if (GET_INVENTORY_GS_TOKENS < 100) {
            Actor_Kill(&this->actor);
            // "Gold Skulltula Shop I still can't be a human"
            osSyncPrintf("金スタル屋 まだ 人間に戻れない \n");
            return;
        }
    } else if (GET_INVENTORY_GS_TOKENS < (this->actor.params * 10)) {
        Actor_Kill(&this->actor);
        // "Gold Skulltula Shop I still can't be a human"
...
void EnSth_SetupAfterObjectLoaded(EnSth* this, PlayState* play) {
    ...
    if (GET_EVENT_CHK_INF[EVENTCHKINF_DA_DB_DC_DD_DE_INDEX] & this->eventFlag) {
...
void EnSth_GiveReward(EnSth* this, PlayState* play) {
    if (Actor_HasParent(&this->actor, play)) {
        ...
        EnSth_SetupAction(this, EnSth_RewardObtainedTalk);
        GET_EVENT_CHK_INF[EVENTCHKINF_DA_DB_DC_DD_DE_INDEX] |= this->eventFlag;
    } else {
