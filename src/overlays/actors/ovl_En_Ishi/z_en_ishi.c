...
static ColliderCylinderSrc sCylinderInits[2] = {
    {
        {
            COL_MATERIAL_HARD,
            AT_NONE,
...
static CollideDataInit sCollideDataInit = { 0, 12, 60, 0xFF };

void EnIshi_InitCollider(Actor* thisx, GlobalContext* globalCtx) {
    EnIshi* this = THIS;

    Collider_InitCylinder(globalCtx, &this->collider);
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInits[this->actor.params & 1]);
    ...
    if (Actor_HasParent(&this->actor, globalCtx)) {
        ...
        if ((this->actor.params >> 4) & 1) {
            EnIshi_SpawnBugs(this, globalCtx);
               ...
               this->collider.elem.acHitElem->atDmgInfo.dmgFlags & 0x40000048) {
