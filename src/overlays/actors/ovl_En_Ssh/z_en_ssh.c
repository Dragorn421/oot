...
static CollideDataInitAlt sCollideDataInit = { 1, 0, 0, 0, 0xFF };
...
void EnSsh_InitColliders(EnSsh* this, GlobalContext* globalCtx) {
    ...
    this->colCylinder[0].elem.acDmgInfo.dmgFlags = 0x0003F8E9;
    this->colCylinder[1].elem.acDmgInfo.dmgFlags = 0xFFC00716;
    this->colCylinder[2].base.colMaterial = 9;
    this->colCylinder[2].elem.acElemFlags = 0xD;
    this->colCylinder[2].elem.elemMaterial = 2;
    this->colCylinder[2].elem.acDmgInfo.dmgFlags = 0xFFCC0716;

    func_80061EFC(&this->actor.collideData, CollisionBtlTbl_Get(2), &sCollideDataInit);

    Collider_InitSpheres(globalCtx, &this->colSph);
    Collider_LoadSpheres(globalCtx, &this->colSph, &this->actor, &sJntSphInit, this->colSphElements);
}
...
s32 EnSsh_CheckHitLink(EnSsh* this, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < 3; i++) {
        if (this->colCylinder[i + 3].base.ocFlags2 & 1) {
            this->colCylinder[i + 3].base.ocFlags2 &= ~1;
            hit = true;
        }
    }
    if (!hit) {
...
void EnSsh_SetColliders(EnSsh* this, GlobalContext* globalCtx) {
    if (this->actor.collideData.health == 0) {
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colSph.base);
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colSph.base);
    } else {
        if (this->hitTimer == 0) {
