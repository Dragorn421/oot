...
static CollideDataInit sCollideDataInit = { 0xA, 0x23, 0x64, 0xFE };
...
void EnHorseZelda_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinderAlt(globalCtx, &this->colliderCylinder, &this->actor, &sCylinderInit);
    Collider_InitSpheres(globalCtx, &this->colliderSphere);
    Collider_LoadSpheres(globalCtx, &this->colliderSphere, &this->actor, &sJntSphInit, &this->colliderSphereItem);
...
void EnHorseZelda_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->actor.posRot2.pos = this->actor.posRot.pos;
    this->actor.posRot2.pos.y += 70.0f;
    Collider_UpdateCylinderShape(&this->actor, &this->colliderCylinder);
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderCylinder.base);
}

void func_80A6DFD4(EnHorseZelda* this, GlobalContext* globalCtx, ColliderSpheresElement* colliderSphereItem) {
    Vec3f sp4C;
    Vec3f sp40;
    s32 pad;
    s32 i;

    for (i = 0; i < this->colliderSphere.nElements; i++) {
        sp4C.x = this->colliderSphere.elements[i].shape.model.center.x;
        sp4C.y = this->colliderSphere.elements[i].shape.model.center.y;
        sp4C.z = this->colliderSphere.elements[i].shape.model.center.z;

        func_800A6408(colliderSphereItem, this->colliderSphere.elements[i].shape.unk14, &sp4C, &sp40);

        this->colliderSphere.elements[i].shape.world.center.x = sp40.x;
        this->colliderSphere.elements[i].shape.world.center.y = sp40.y;
        this->colliderSphere.elements[i].shape.world.center.z = sp40.z;

        this->colliderSphere.elements[i].shape.world.radius =
            this->colliderSphere.elements[i].shape.model.radius * this->colliderSphere.elements[i].shape.unkRadiusScale;
    }
