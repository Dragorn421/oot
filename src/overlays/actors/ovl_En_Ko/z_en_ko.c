...
void func_80A99048(EnKo* this, PlayState* play) {
    if (EnKo_IsOsAnimeLoaded(this, play) && EnKo_AreObjectsLoaded(this, play)) {
        ...
        gSegments[6] = PhysicalAddr(play->objectCtx.slots[this->osAnimeObjectSlot].segment);
...
s32 EnKo_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx, Gfx** gfx) {
    ...
    if (limbIndex == 15) {
        ...
        gSegments[6] = PhysicalAddr(play->objectCtx.slots[this->headObjectSlot].segment);
        ...
        *dList = sHead[headId].dList;
        ...
        gSegments[6] = PhysicalAddr(play->objectCtx.slots[this->legsObjectSlot].segment);
    }
    if (limbIndex == 8) {
...
void EnKo_PostLimbDraw(PlayState* play2, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    ...
    if (limbIndex == 7) {
        ...
        gSegments[6] = PhysicalAddr(play->objectCtx.slots[this->bodyObjectSlot].segment);
    }
    if (limbIndex == 15) {
