...
static ActorDamageChart D_809EC620 = { {
...
void EnDh_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->actor.collideData.damageChart = &D_809EC620;
    ...
    this->actor.collideData.health = (gSaveContext.linkAge == 0) ? 14 : 20;
    this->alpha = this->unk_258 = 255;
    this->actor.flags &= ~1;
    Collider_InitCylinder(globalCtx, &this->collider1);
    Collider_LoadCylinder(globalCtx, &this->collider1, &this->actor, &sCylinderInit);
    Collider_InitSpheres(globalCtx, &this->collider2);
    Collider_LoadSpheres(globalCtx, &this->collider2, &this->actor, &sJntSphInit, this->elements);
    EnDh_SetupWait(this);
}
...
void EnDh_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->collider2);
}
    ...
    switch (this->actionState) {
        ...
        case 2:
            if (this->skelAnime.animCurrentFrame >= 4.0f) {
                this->collider2.base.atFlags = this->collider2.elements[0].base.atElemFlags = 0x11;
                this->collider2.elements[0].base.atDmgInfo.dmgFlags = 0xFFCFFFFF;
...
void EnDh_Burrow(EnDh* this, GlobalContext* globalCtx) {
    switch (this->actionState) {
        case 0:
            this->actionState++;
            this->drawDirtWave++;
            this->collider1.base.atFlags = this->collider1.elem.atElemFlags = 0x11;
            this->collider1.elem.atDmgInfo.dmgFlags = 0xFFCFFFFF;
        ...
        case 1:
            ...
            this->dirtWaveAlpha = (s16)(Math_Sins(this->dirtWavePhase) * 255.0f);
            EnDh_SpawnDebris(globalCtx, this, &this->actor.posRot.pos, this->dirtWaveSpread, 4, 2.05f, 1.2f);
            this->collider1.shape.radius = this->dirtWaveSpread * 0.6f;
            if (SkelAnime_FrameUpdateMatrix(&this->skelAnime)) {
                this->actionState++;
            }
            break;
        case 2:
            this->drawDirtWave = false;
            this->collider1.shape.radius = 35;
            this->collider1.base.atFlags = this->collider1.elem.atElemFlags = 0;
...
void EnDh_CollisionCheck(EnDh* this, GlobalContext* globalCtx) {
    ...
    if ((this->collider2.base.acFlags & 2) && !this->retreat) {
        ...
        if ((this->actor.collideData.damageEffect != 0) && (this->actor.collideData.damageEffect != 6)) {
            ...
            lastHealth = this->actor.collideData.health;
            if (Actor_ApplyDamage(&this->actor) == 0) {
                EnDh_SetupDeath(this);
                Item_DropCollectibleRandom(globalCtx, &this->actor, &this->actor.posRot.pos, 0x90);
            } else {
                if (((lastHealth >= 15) && (this->actor.collideData.health < 15)) ||
                    ((lastHealth >= 9) && (this->actor.collideData.health < 9)) ||
                    ((lastHealth >= 3) && (this->actor.collideData.health < 3))) {
...
void EnDh_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_UpdateCylinderShape(&this->actor, &this->collider1);
    if (this->actor.collideData.health > 0) {
        ...
        if (((this->curAction != DH_DAMAGE) && (this->actor.shape.unk_08 == 0.0f)) ||
            ...
            Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->collider2.base);
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->collider2.base);
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->collider1.base);
        }
    } else {
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->collider1.base);
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->collider2.base);
    }
}
...
void EnDh_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    ...
    if (limbIndex == 13) {
        ...
        func__800628A4_Type0(1, &this->collider2);
        Matrix_Pull();
    }
}
