...
static ColliderTrisElementSrc D_80B9EC34[2] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static ColliderTrisSrc sRustyFloorTrisInit = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
...
static ColliderTrisElementSrc D_80B9ECBC[2] = {
    {
        {
            ELEM_MATERIAL_UNK4,
            {
...
static ColliderTrisSrc trisColliderEye = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
...
static ColliderSpheresElementSrc D_80B9ED44[1] = {
    {
...
static ColliderSpheresSrc sCyrstalJntSphereInit = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
...
void ObjSwitch_InitJntSphCollider(ObjSwitch* this, GlobalContext* globalCtx, ColliderSpheresSrc* colliderJntSphInit) {
    ColliderSpheres* colliderJntSph = &this->jntSph.col;

    Collider_InitSpheres(globalCtx, colliderJntSph);
    Collider_LoadSpheres(globalCtx, colliderJntSph, &this->dyna.actor, colliderJntSphInit, this->jntSph.items);
    ...
    func__800628A4_Type0(0, colliderJntSph);
}

void ObjSwitch_InitTrisCollider(ObjSwitch* this, GlobalContext* globalCtx, ColliderTrisSrc* colliderTrisInit) {
    ...
    Collider_LoadTris(globalCtx, colliderTris, &this->dyna.actor, colliderTrisInit, this->tris.items);

    for (i = 0; i < 2; i++) {
        for (j = 0; j < 3; j++) {
            ObjSwitch_RotateY(&pos[j], &colliderTrisInit->elements[i].vertices[j], this->dyna.actor.initPosRot.rot.y);
...
void ObjSwitch_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->dyna.actor.collideData.mass = 0xFF;
...
void ObjSwitch_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch ((this->dyna.actor.params & 7)) {
        ...
        case OBJSWITCH_TYPE_CRYSTAL_TARGETABLE:
            Collider_DestroySpheres(globalCtx, &this->jntSph.col);
            break;
    }
}
...
void ObjSwitch_FloorUp(ObjSwitch* this, GlobalContext* globalCtx) {
    if ((this->dyna.actor.params & 7) == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        if (this->tris.col.base.acFlags & 2) {
            ...
            Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->tris.col.base);
        }
    } else {
    ...
    switch ((this->dyna.actor.params & 7)) {
        ...
        case OBJSWITCH_TYPE_CRYSTAL_TARGETABLE:
            ...
            if (this->disableAcTimer <= 0) {
                Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->jntSph.col.base);
            }
            Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->jntSph.col.base);
            break;
    }
}
