...
ColliderCylinderSrc sBodyCylInit_EnGeldb = {
    {
        COL_MATERIAL_HIT5,
        AT_NONE,
...
ColliderTrisElementSrc sBlockTrisElementsInit_EnGeldb[2] = {
    {
        {
            ELEM_MATERIAL_UNK2,
            {
...
ColliderTrisSrc sBlockTrisInit_EnGeldb = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
    ...
    2,
    sBlockTrisElementsInit_EnGeldb,
};

ColliderQuadSrc sSwordQuadinit_EnGeldb = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
void EnGeldB_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisx->collideData.damageChart = &sActorDamageChart;
    ActorShape_Init(&thisx->shape, 0.0f, ActorShadow_DrawFunc_Teardrop, 0.0f);
    this->actor.collideData.mass = 0xFE;
    thisx->collideData.health = 20;
    thisx->collideData.unk_10 = 50;
    thisx->collideData.unk_12 = 100;
    ...
    Collider_LoadCylinder(globalCtx, &this->bodyCollider, thisx, &sBodyCylInit_EnGeldb);
    Collider_InitTris(globalCtx, &this->blockCollider);
    Collider_LoadTris(globalCtx, &this->blockCollider, thisx, &sBlockTrisInit_EnGeldb, this->blockElements);
    Collider_InitQuad(globalCtx, &this->swordCollider);
    Collider_LoadQuad(globalCtx, &this->swordCollider, thisx, &sSwordQuadinit_EnGeldb);
...
void func_80A392D8(EnGeldB* this, GlobalContext* globalCtx) {
    ...
    if (this->blockCollider.base.acFlags & 0x80) {
        ...
        this->bodyCollider.base.acFlags &= ~2;
               ...
               (this->spinAttackState < 2)) {
        this->bodyCollider.base.acFlags &= ~2;
        if (this->actor.collideData.damageEffect != GELDB_DAMAGE_UNK6) {
            this->damageEffect = this->actor.collideData.damageEffect;
            func_80035650(&this->actor, &this->bodyCollider.elem, 1);
            func_800F8A44(&this->actor.projectedPos, NA_SE_EN_GERUDOFT_BREATH);
            if ((this->actor.collideData.damageEffect == GELDB_DAMAGE_STUN) ||
                (this->actor.collideData.damageEffect == GELDB_DAMAGE_FREEZE)) {
...
void EnGeldB_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_UpdateCylinderShape(&this->actor, &this->bodyCollider);
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->bodyCollider.base);
    if ((this->actionState >= GELDB_READY) && (this->spinAttackState < 2) &&
        ((this->actor.dmgEffectTimer == 0) || !(this->actor.dmgEffectParams & 0x4000))) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->bodyCollider.base);
    }
    if ((this->actionState == GELDB_BLOCK) && (this->skelAnime.curFrame == 0.0f)) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->blockCollider.base);
    }
    if (this->swordState > 0) {
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->swordCollider.base);
    }
...
void EnGeldB_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 11) {
        Matrix_MultVec3f(&swordQuadOffset1, &this->swordCollider.shape.corners.cornerC);
        Matrix_MultVec3f(&swordQuadOffset0, &this->swordCollider.shape.corners.cornerD);
        Matrix_MultVec3f(&swordQuadOffset3, &this->swordCollider.shape.corners.cornerB);
        Matrix_MultVec3f(&swordQuadOffset2, &this->swordCollider.shape.corners.cornerA);
        func_80062734(&this->swordCollider, &this->swordCollider.shape.corners.cornerD,
                      &this->swordCollider.shape.corners.cornerC, &this->swordCollider.shape.corners.cornerA,
                      &this->swordCollider.shape.corners.cornerB);
