...
void func_808B4C4C(BgSpot16Bombstone* this, GlobalContext* globalCtx) {
    ...
    Collider_InitSpheres(globalCtx, &this->colliderJntSph);
    Collider_LoadSpheres(globalCtx, &this->colliderJntSph, &this->actor, &sJntSphInit, this->colliderJntSphItems);
    this->colliderJntSph.elements[0].shape.world.center.x = this->actor.posRot.pos.x;
    this->colliderJntSph.elements[0].shape.world.center.y = this->actor.posRot.pos.y + 50.0f;
    this->colliderJntSph.elements[0].shape.world.center.z = this->actor.posRot.pos.z;
    this->colliderJntSph.elements[0].shape.world.radius = 120;
}
...
void func_808B4D04(BgSpot16Bombstone* this, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->colliderCylinder, &this->actor, &sCylinderInit);
    this->colliderCylinder.shape.pos.x += (s16)this->actor.posRot.pos.x;
    this->colliderCylinder.shape.pos.y += (s16)this->actor.posRot.pos.y;
    this->colliderCylinder.shape.pos.z += (s16)this->actor.posRot.pos.z;
}
...
void BgSpot16Bombstone_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params == 0xFF) {
        ...
        Collider_DestroySpheres(globalCtx, &this->colliderJntSph);
...
void func_808B5950(BgSpot16Bombstone* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderCylinder.base.acFlags & 2) {
        ...
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderCylinder);
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderJntSph);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderJntSph);
    }
