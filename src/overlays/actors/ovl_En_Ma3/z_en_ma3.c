...
u16 func_80AA2AA0(PlayState* play, Actor* thisx) {
    ...
    timer1ValuePtr = &GET_TIMER1VALUE;
    if (GET_EVENTINF(EVENTINF_HORSES_0A)) {
        SET_TIMER1VALUE(GET_TIMER1VALUE);
        thisx->flags |= ACTOR_FLAG_16;
        if (GET_TIMER1VALUE >= 0xD3) {
            return 0x208E;
        }
        if ((HIGH_SCORE(HS_HORSE_RACE) == 0) || (HIGH_SCORE(HS_HORSE_RACE) >= 0xB4)) {
            HIGH_SCORE(HS_HORSE_RACE) = 0xB4;
            SET_TIMER1VALUE(*timer1ValuePtr);
        }
        if (!GET_EVENTCHKINF(EVENTCHKINF_1E) && (GET_TIMER1VALUE < 0x32)) {
            return 0x208F;
        } else if (GET_TIMER1VALUE < HIGH_SCORE(HS_HORSE_RACE)) {
            return 0x2012;
        } else {
            return 0x2004;
        }
    ...
    switch (Message_GetState(&play->msgCtx)) {
        ...
        case TEXT_STATE_CLOSING:
            switch (thisx->textId) {
                ...
                case 0x2004:
                case 0x2012:
                    if (HIGH_SCORE(HS_HORSE_RACE) > GET_TIMER1VALUE) {
                        HIGH_SCORE(HS_HORSE_RACE) = GET_TIMER1VALUE;
                    }
                    FALLTHROUGH;
                case 0x208E:
