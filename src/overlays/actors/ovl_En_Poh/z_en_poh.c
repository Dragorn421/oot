...
static ColliderSpheresElementSrc D_80AE1AA0[1] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static CollideDataInit sCollideDataInit = { 0x04, 0x0019, 0x0032, 0x28 };
...
void EnPoh_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_InitSpheres(globalCtx, &this->colliderSph);
    Collider_LoadSpheres(globalCtx, &this->colliderSph, &this->actor, &sJntSphInit, &this->colliderSphItem);
    this->colliderSph.elements[0].shape.world.radius = 0;
    this->colliderSph.elements[0].shape.world.center.x = this->actor.posRot.pos.x;
    this->colliderSph.elements[0].shape.world.center.y = this->actor.posRot.pos.y;
    this->colliderSph.elements[0].shape.world.center.z = this->actor.posRot.pos.z;
    Collider_InitCylinder(globalCtx, &this->colliderCyl);
    Collider_LoadCylinder(globalCtx, &this->colliderCyl, &this->actor, &sCylinderInit);
    func_80061ED4(&this->actor.collideData, &sActorDamageChart, &sCollideDataInit);
    this->unk_194 = 0;
    this->unk_195 = 32;
...
void EnPoh_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->colliderSph);
...
void func_80ADE28C(EnPoh* this) {
    ...
    if (this->colliderCyl.elem.acHitElem->atDmgInfo.dmgFlags & 0x0001F824) {
...
void EnPoh_Talk(EnPoh* this, GlobalContext* globalCtx) {
    ...
    this->colliderCyl.shape.radius = 13;
    this->colliderCyl.shape.height = 30;
    this->colliderCyl.shape.yShift = 0;
    this->colliderCyl.shape.pos.x = this->actor.posRot.pos.x;
    this->colliderCyl.shape.pos.y = this->actor.posRot.pos.y - 20.0f;
    this->colliderCyl.shape.pos.z = this->actor.posRot.pos.z;
    this->colliderCyl.base.ocFlags1 = 9;
...
void func_80ADFE80(EnPoh* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderCyl.base.ocFlags1 & 2) {
        this->actor.flags |= 0x10000;
        func_8002F2F4(&this->actor, globalCtx);
    } else {
        this->actor.flags &= ~0x10000;
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderCyl.base);
    }
...
void EnPoh_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Object_IsLoaded(&globalCtx->objectCtx, this->objectIdx) != 0) {
        ...
        if (this->infoIdx == EN_POH_INFO_NORMAL) {
            ...
            this->colliderSph.elements[0].shape.unk14 = 9;
            this->colliderSph.elements[0].shape.model.center.y *= -1;
            this->actor.shape.rot.y = this->actor.posRot.rot.y = -0x4000;
            this->colliderCyl.shape.radius = 20;
            this->colliderCyl.shape.height = 55;
            this->colliderCyl.shape.yShift = 15;
        }
        this->actor.flags &= ~0x10;
...
void EnPoh_UpdateLiving(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actionFunc == EnPoh_Attack && this->unk_198 < 10) {
        ...
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderSph.base);
    }
    Collider_UpdateCylinderShape(&this->actor, &this->colliderCyl);
    if ((this->colliderCyl.base.acFlags & 1) && this->lightColor.a == 255) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderCyl.base);
    }
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderCyl.base);
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderSph.base);
...
void EnPoh_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP) {
    ...
    func__800628A4_Type0(limbIndex, &this->colliderSph);
    ...
    if (limbIndex == this->info->unk_6) {
        ...
        Lights_PointGlowSetInfo(&this->lightInfo, this->colliderSph.elements[0].shape.world.center.x,
                                this->colliderSph.elements[0].shape.world.center.y,
                                this->colliderSph.elements[0].shape.world.center.z, this->envColor.r, this->envColor.g,
