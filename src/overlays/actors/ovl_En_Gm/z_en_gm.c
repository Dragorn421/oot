...
void func_80A3DB04(EnGm* this, GlobalContext* globalCtx) {
    ...
    } else if ((this->collider.base.ocFlags1 & 2) || (SQ(dx) + SQ(dz)) < SQ(100.0f)) {
...
void func_80A3DC44(EnGm* this, GlobalContext* globalCtx) {
    ...
    if ((this->collider.base.ocFlags1 & 2) || (SQ(dx) + SQ(dz)) < SQ(100.0f)) {
        this->collider.base.acFlags &= ~2;
        func_8002F2CC(&this->actor, globalCtx, 415.0f);
    }
}
...
void func_80A3E090(EnGm* this) {
    ...
    this->collider.shape.pos.x = vec2.x;
    this->collider.shape.pos.y = vec2.y;
    this->collider.shape.pos.z = vec2.z;
    Matrix_Pull();
    Matrix_Push();
