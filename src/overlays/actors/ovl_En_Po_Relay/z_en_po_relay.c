...
void EnPoRelay_DisappearAndReward(EnPoRelay* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.scale.x, 0.0f, 0.001f) != 0) {
        if (this->hookshotSlotFull != 0) {
            ...
            if (GET_TIMER1VALUE < HIGH_SCORE(HS_DAMPE_RACE)) {
                HIGH_SCORE(HS_DAMPE_RACE) = GET_TIMER1VALUE;
            }
            if (Flags_GetCollectible(play, this->actor.params) == 0 && GET_TIMER1VALUE <= 60) {
