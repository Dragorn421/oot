...
void BgHidanHamstep_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->dyna.actor.params & 0xFF) == 0) {
        ...
        Collider_LoadTris(globalCtx, &this->collider, &this->dyna.actor, &sTrisInit, this->colliderItems);

        for (i = 0; i < 2; i++) {
            for (i2 = 0; i2 < 3; i2++) {
                sp48[i2].x = sTrisInit.elements[i].vertices[i2].x + this->dyna.actor.initPosRot.pos.x;
                sp48[i2].y = sTrisInit.elements[i].vertices[i2].y + this->dyna.actor.initPosRot.pos.y;
                sp48[i2].z = sTrisInit.elements[i].vertices[i2].z + this->dyna.actor.initPosRot.pos.z;
            }
