...
static ColliderCylinderSrc sCylInitStand = {
    {
        COL_MATERIAL_METAL,
        AT_NONE,
...
static ColliderCylinderSrc sCylInitFlame = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
...
void ObjSyokudai_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->colliderStand, &this->actor, &sCylInitStand);
    ...
    Collider_LoadCylinder(globalCtx, &this->colliderFlame, &this->actor, &sCylInitFlame);
...
void ObjSyokudai_Update(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (func_80042244(globalCtx, &globalCtx->colCtx, this->actor.posRot.pos.x, this->actor.posRot.pos.z, &waterSurface,
        ...
        if (this->colliderFlame.base.acFlags & 2) {
            toucherFlags = this->colliderFlame.elem.acHitElem->atDmgInfo.dmgFlags;
    ...
    Collider_UpdateCylinderShape(&this->actor, colliderStand);
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &colliderStand->base);
    Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &colliderStand->base);

    colliderFlame = &this->colliderFlame;
    Collider_UpdateCylinderShape(&this->actor, colliderFlame);
    Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &colliderFlame->base);
