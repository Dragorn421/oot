...
typedef enum ArmosDamageReaction {
    /*  0 */ AM_DMG_REACTION_NONE, // used by anything that can't kill the armos
    /*  1 */ AM_DMG_REACTION_NUT,
    /*  6 */ AM_DMG_REACTION_STUN = 6, // doesn't include deku nuts
    /* 13 */ AM_DMG_REACTION_ICE = 13,
    /* 14 */ AM_DMG_REACTION_MAGIC_FIRE_LIGHT,
    /* 15 */ AM_DMG_REACTION_KILL // any damage source that can kill the armos (and isn't a special case)
} ArmosDamageReaction;

static DamageTable sDamageTable = {
    /* Deku nut      */ DMG_ENTRY(0, AM_DMG_REACTION_NUT),
    /* Deku stick    */ DMG_ENTRY(2, AM_DMG_REACTION_NONE),
    /* Slingshot     */ DMG_ENTRY(1, AM_DMG_REACTION_NONE),
    /* Explosive     */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Boomerang     */ DMG_ENTRY(0, AM_DMG_REACTION_STUN),
    /* Normal arrow  */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Hammer swing  */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Hookshot      */ DMG_ENTRY(0, AM_DMG_REACTION_STUN),
    /* Kokiri sword  */ DMG_ENTRY(1, AM_DMG_REACTION_NONE),
    /* Master sword  */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Giant's Knife */ DMG_ENTRY(4, AM_DMG_REACTION_KILL),
    /* Fire arrow    */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Ice arrow     */ DMG_ENTRY(4, AM_DMG_REACTION_ICE),
    /* Light arrow   */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Unk arrow 1   */ DMG_ENTRY(2, AM_DMG_REACTION_NONE),
    /* Unk arrow 2   */ DMG_ENTRY(2, AM_DMG_REACTION_NONE),
    /* Unk arrow 3   */ DMG_ENTRY(2, AM_DMG_REACTION_NONE),
    /* Fire magic    */ DMG_ENTRY(0, AM_DMG_REACTION_MAGIC_FIRE_LIGHT),
    /* Ice magic     */ DMG_ENTRY(3, AM_DMG_REACTION_ICE),
    /* Light magic   */ DMG_ENTRY(0, AM_DMG_REACTION_MAGIC_FIRE_LIGHT),
    /* Shield        */ DMG_ENTRY(0, AM_DMG_REACTION_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, AM_DMG_REACTION_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, AM_DMG_REACTION_NONE),
    /* Giant spin    */ DMG_ENTRY(4, AM_DMG_REACTION_KILL),
    /* Master spin   */ DMG_ENTRY(2, AM_DMG_REACTION_KILL),
    /* Kokiri jump   */ DMG_ENTRY(2, AM_DMG_REACTION_NONE),
    /* Giant jump    */ DMG_ENTRY(8, AM_DMG_REACTION_KILL),
    /* Master jump   */ DMG_ENTRY(4, AM_DMG_REACTION_KILL),
    /* Unknown 1     */ DMG_ENTRY(0, AM_DMG_REACTION_NONE),
    /* Unblockable   */ DMG_ENTRY(0, AM_DMG_REACTION_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, AM_DMG_REACTION_KILL),
    /* Unknown 2     */ DMG_ENTRY(0, AM_DMG_REACTION_NONE),
};
...
void EnAm_SetupStunned(EnAm* this, PlayState* play) {
    ...
    if (this->damageReaction == AM_DMG_REACTION_ICE) {
        this->iceTimer = 48;
    }
...
void EnAm_UpdateDamage(EnAm* this, PlayState* play) {
    ...
    if (this->deathTimer == 0) {
        if (this->blockCollider.base.acFlags & AC_BOUNCED) {
            ...
            if (this->dyna.actor.colChkInfo.damageReaction != AM_DMG_REACTION_MAGIC_FIRE_LIGHT) {
                this->unk_264 = 0;
                ...
                if ((this->dyna.actor.colChkInfo.damageReaction == AM_DMG_REACTION_NUT) ||
                    (this->dyna.actor.colChkInfo.damageReaction == AM_DMG_REACTION_STUN) ||
                    (this->dyna.actor.colChkInfo.damageReaction == AM_DMG_REACTION_ICE)) {
                    ...
                    } else if (this->dyna.actor.colChkInfo.damageReaction == AM_DMG_REACTION_STUN) {
                        Vec3f sparkPos = this->dyna.actor.world.pos;

                        sparkPos.y += 50.0f;
                        CollisionCheck_SpawnShieldParticlesMetal(play, &sparkPos);
                    }
                } else if ((this->dyna.actor.colChkInfo.damageReaction == AM_DMG_REACTION_KILL) ||
                           (this->behavior == AM_BEHAVIOR_STUNNED)) {
                    this->dyna.actor.colChkInfo.health = 0;
