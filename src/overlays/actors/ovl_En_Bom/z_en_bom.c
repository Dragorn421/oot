...
void EnBom_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->actor.collideData.mass = 200;
    this->actor.collideData.unk_10 = 5;
    this->actor.collideData.unk_12 = 0xA;
    this->timer = 70;
    this->flashSpeedScale = 7;
    Collider_InitCylinder(globalCtx, &this->bombCollider);
    Collider_InitSpheres(globalCtx, &this->explosionCollider);
    Collider_LoadCylinder(globalCtx, &this->bombCollider, &this->actor, &sCylinderInit);
    Collider_LoadSpheres(globalCtx, &this->explosionCollider, &this->actor, &sJntSphInit,
...
void EnBom_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->explosionCollider);
...
void EnBom_Explode(EnBom* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.params == BOMB_EXPLOSION) {
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->explosionCollider.base);
    }
...
void EnBom_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params == BOMB_BODY) {
        ...
        if ((this->bombCollider.base.acFlags & 2) ||
            ((this->bombCollider.base.ocFlags1 & 2) && (this->bombCollider.base.oc->type == ACTORTYPE_ENEMY))) {
            this->timer = 0;
    ...
    if (this->actor.params <= BOMB_BODY) {
        Collider_UpdateCylinderShape(&this->actor, &this->bombCollider);
        ...
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->bombCollider.base);
    }
...
void EnBom_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params == BOMB_BODY) {
        ...
        func__800628A4_Type0(0, &this->explosionCollider);
    }
