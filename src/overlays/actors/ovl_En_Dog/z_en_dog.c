...
s8 EnDog_CanFollow(EnDog* this, PlayState* play) {
    ...
    if (this->collider.base.ocFlags2 & OC2_HIT_PLAYER) {
        ...
        SET_DOG_PARAMS((this->actor.params & 0x7FFF))
        return 1;
    }
...
void EnDog_Init(Actor* thisx, PlayState* play) {
    ...
    followingDog = ((GET_DOG_PARAMS & 0x0F00) >> 8);
    ...
    switch (play->sceneId) {
        case SCENE_MARKET_NIGHT:
            if ((!GET_DOG_IS_LOST) && (((this->actor.params & 0x0F00) >> 8) == 1)) {
                Actor_Kill(&this->actor);
            }
            break;
...
void EnDog_Wait(EnDog* this, PlayState* play) {
    ...
    if ((GET_DOG_PARAMS != 0) && (this->actor.xzDistToPlayer < 200.0f)) {
        this->nextBehavior = DOG_RUN;
        this->actionFunc = EnDog_RunAway;
    }
}
