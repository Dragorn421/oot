...
void DoorAna_WaitOpen(DoorAna* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.scale.x, 0.01f, 0.001f)) {
        if ((this->actor.targetMode != 0) && (play->transitionTrigger == TRANS_TRIGGER_OFF) &&
            ...
            SET_RESPAWN_A0_POS_Y(RESPAWN_MODE_RETURN, this->actor.world.pos.y);
            SET_RESPAWN_A0_YAW(RESPAWN_MODE_RETURN, this->actor.home.rot.y);
            SET_RESPAWN_A0_DATA(RESPAWN_MODE_RETURN, this->actor.params & 0xFFFF);
            if (destinationIdx < 0) {
