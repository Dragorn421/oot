...
void BgSpot00Hanebasi_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->dyna.actor.params == DT_DRAWBRIDGE) {
        ...
        if ((GET_SCENE_LAYER != 6) &&
            ((GET_SCENE_LAYER == 4) || (GET_SCENE_LAYER == 5) || (!LINK_IS_ADULT && !IS_DAY))) {
    ...
    if (this->dyna.actor.params == DT_DRAWBRIDGE) {
        ...
        if (GET_SCENE_LAYER == 5) {
            ...
            if (gTimeSpeed == 50) {
                ...
                if (GET_DAY_TIME > CLOCK_TIME(20, 0) + 1) {
                    tmp = CLOCK_TIME(20, 0) + 1 + 0x10000;
                }

                gTimeSpeed = (tmp - GET_DAY_TIME) * (1.0f / 350.0f);
            }

            dayTime = GET_DAY_TIME;

            if ((dayTime > CLOCK_TIME(4, 0)) && (dayTime < CLOCK_TIME(4, 30)) && (GET_SCENE_LAYER == 5)) {
                gTimeSpeed = 0;
            }
        }
    }
