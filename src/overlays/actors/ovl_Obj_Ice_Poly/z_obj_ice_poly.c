...
static ColliderCylinderSrc sCylinderInitIce = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
...
static ColliderCylinderSrc sCylinderInitHard = {
    {
        COL_MATERIAL_HARD,
        AT_NONE,
...
void ObjIcePoly_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->colliderIce, thisx, &sCylinderInitIce);
    Collider_InitCylinder(globalCtx, &this->colliderHard);
    Collider_LoadCylinder(globalCtx, &this->colliderHard, thisx, &sCylinderInitHard);
    Collider_UpdateCylinderShape(thisx, &this->colliderIce);
    Collider_UpdateCylinderShape(thisx, &this->colliderHard);
    thisx->collideData.mass = 0xFF;
    this->alpha = 255;
    this->colliderIce.shape.radius *= thisx->scale.x;
    this->colliderIce.shape.height *= thisx->scale.y;
    this->colliderHard.shape.radius *= thisx->scale.x;
    this->colliderHard.shape.height *= thisx->scale.y;
...
void ObjIcePoly_Idle(ObjIcePoly* this, GlobalContext* globalCtx) {
    ...
    if (this->colliderIce.base.acFlags & 2) {
        ...
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->colliderIce.base);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderIce.base);
        Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->colliderIce.base);
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->colliderHard.base);
    } else {
