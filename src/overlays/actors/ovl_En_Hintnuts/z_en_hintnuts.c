...
void EnHintnuts_HitByScrubProjectile2(EnHintnuts* this) {
    ...
    this->collider.shape.height = 0x25;
...
void EnHintnuts_Wait(EnHintnuts* this, GlobalContext* globalCtx) {
    ...
    this->collider.shape.height = (((boundedCurrentFrame - 9.0f) * 9.0f) + 5.0f);
...
void EnHintnuts_Burrow(EnHintnuts* this, GlobalContext* globalCtx) {
    ...
    if (SkelAnime_Update(&this->skelAnime) != 0) {
        ...
        boundedCurrentFrame = this->skelAnime.curFrame < 1.0f   ? 1.0f
                              : this->skelAnime.curFrame > 3.0f ? 3.0f
                                                                : this->skelAnime.curFrame;
        this->collider.shape.height = (((3.0f - boundedCurrentFrame) * 12.0f) + 5.0f);
    }
...
void EnHintnuts_CheckProximity(EnHintnuts* this, GlobalContext* globalCtx) {
    if (this->actor.type != ACTORTYPE_ENEMY) {
        if ((this->collider.base.ocFlags1 & 2) || (this->actor.unk_10C != 0)) {
...
void EnHintnuts_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params != 0xA) {
        ...
        if (this->actionFunc != EnHintnuts_Freeze && this->actionFunc != EnHintnuts_BeginFreeze) {
            ...
            func_8002E4B4(globalCtx, &this->actor, 20.0f, this->collider.shape.radius, this->collider.shape.height,
                          0x1D);
        }
