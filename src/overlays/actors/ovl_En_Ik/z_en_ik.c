...
static ColliderTrisElementSrc sTrisItemsInit[2] = {
    {
        {
            ELEM_MATERIAL_UNK2,
            {
...
void func_80A74398(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->bodyCollider, thisx, &sCylinderInit);
    Collider_InitTris(globalCtx, &this->shieldCollider);
    Collider_LoadTris(globalCtx, &this->shieldCollider, thisx, &sTrisInit, this->shieldColliderItems);
    Collider_InitQuad(globalCtx, &this->axeCollider);
    Collider_LoadQuad(globalCtx, &this->axeCollider, thisx, &sQuadInit);

    thisx->collideData.damageChart = &sActorDamageChart;
    thisx->collideData.mass = 0xFE;
    this->unk_2FC = 0;
    thisx->collideData.health = 30;
    thisx->gravity = -1.0f;
...
void func_80A75A38(EnIk* this, GlobalContext* globalCtx) {
    if (SkelAnime_Update(&this->skelAnime)) {
        if ((this->actor.collideData.health == 0) && (this->unk_2F9 != 0)) {
            s32 i;
            Vec3f pos;
...
void func_80A75C38(EnIk* this, GlobalContext* globalCtx) {
    ...
    temp_v0_3 = this->actor.collideData.damageEffect;
    ...
    prevHealth = this->actor.collideData.health;
    Actor_ApplyDamage(&this->actor);
    if (this->actor.params != 0) {
        if ((prevHealth > 10) && (this->actor.collideData.health <= 10)) {
            this->unk_2FB = 1;
            func_80032E24(&this->unk_308, 3, globalCtx);
        }
    } else if (this->actor.collideData.health <= 10) {
...
void func_80A75FA0(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((this->actor.params == 0) && (this->actor.collideData.health <= 10)) {
    ...
    if ((this->actor.collideData.health > 0) && (this->actor.dmgEffectTimer == 0) && (this->unk_2F8 >= 2)) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->bodyCollider.base);
    }
    if (this->unk_2FE > 0) {
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->axeCollider.base);
    }
    if (this->unk_2F8 == 9) {
        Collider_AddAC(globalCtx, &globalCtx->colliderCtx, &this->shieldCollider.base);
    }
...
void EnIk_PostLimbDraw3(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 12) {
        ...
        Matrix_MultVec3f(&D_80A7847C[0], &this->axeCollider.shape.corners.cornerC);
        Matrix_MultVec3f(&D_80A7847C[1], &this->axeCollider.shape.corners.cornerD);
        Matrix_MultVec3f(&D_80A7847C[2], &this->axeCollider.shape.corners.cornerB);
        Matrix_MultVec3f(&D_80A7847C[3], &this->axeCollider.shape.corners.cornerA);
        func_80062734(&this->axeCollider, &this->axeCollider.shape.corners.cornerD,
                      &this->axeCollider.shape.corners.cornerC, &this->axeCollider.shape.corners.cornerA,
                      &this->axeCollider.shape.corners.cornerB);
