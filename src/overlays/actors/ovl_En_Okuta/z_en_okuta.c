...
static ColliderCylinderSrc sProjectileColliderInit = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
...
static ColliderCylinderSrc sOctorockColliderInit = {
    {
        COL_MATERIAL_HIT0,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
...
static CollideDataInit sCollideDataInit = { 1, 0xF, 0x3C, 0x64 };
...
void EnOkuta_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == 0) {
        ...
        Collider_LoadCylinder(globalCtx, &this->collider, thisx, &sOctorockColliderInit);
        func_80061ED4(&thisx->collideData, &sActorDamageChart, &sCollideDataInit);
        ...
        thisx->flags |= 0x10;
        Collider_InitCylinder(globalCtx, &this->collider);
        Collider_LoadCylinder(globalCtx, &this->collider, thisx, &sProjectileColliderInit);
...
void EnOkuta_ProjectileFly(EnOkuta* this, GlobalContext* globalCtx) {
    ...
    if ((this->actor.bgCheckFlags & 8) || (this->actor.bgCheckFlags & 1) || (this->collider.base.atFlags & 2) ||
        this->collider.base.acFlags & 2 || this->collider.base.ocFlags1 & 2 || this->actor.groundY == -32000.0f) {
...
void EnOkuta_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (!(player->stateFlags1 & 0x300000C0)) {
        ...
        if (this->actor.params == 0) {
            ...
            this->collider.shape.height =
                (((sOctorockColliderInit.shape.height * this->headScale.y) - this->collider.shape.yShift) *
        ...
        if ((this->actionFunc == EnOkuta_Appear) || (this->actionFunc == EnOkuta_Hide)) {
            this->collider.shape.pos.y = this->actor.posRot.pos.y + (this->skelAnime.jointTable->y * this->actor.scale.y);
            this->collider.shape.radius = sOctorockColliderInit.shape.radius * this->actor.scale.x * 100.0f;
        }
        if (this->actor.params == 0x10) {
            this->actor.flags |= 0x1000000;
            Collider_AddAT(globalCtx2, &globalCtx2->colliderCtx, &this->collider.base);
        }
