...
void EnRr_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    Collider_LoadCylinderAlt(globalCtx, &this->collider1, &this->actor, &sCylinderInit1);
    Collider_InitCylinder(globalCtx, &this->collider2);
    Collider_LoadCylinderAlt(globalCtx, &this->collider2, &this->actor, &sCylinderInit2);
...
void EnRr_CollisionCheck(EnRr* this, GlobalContext* globalCtx) {
    ...
    if (this->collider2.base.acFlags & 2) {
        ...
        if (this->collider1.base.acFlags & 2) {
            ...
            if (this->actor.collideData.damageEffect != 0) {
                ...
                Collider_BlueBlood(globalCtx, NULL, &hitPos);
            }
            switch (this->actor.collideData.damageEffect) {
                ...
                case RR_DAMAGE_NORMAL:
                    ...
                    osSyncPrintf(VT_FGCOL(RED) "いてっ( %d : LIFE %d : DAMAGE %d : %x )！！" VT_RST "\n",
                                 this->frameCount, this->actor.collideData.health, this->actor.collideData.damage,
                                 this->actor.collideData.damageEffect);
                    this->stopScroll = false;
        ...
        if ((this->ocTimer == 0) && (this->actor.dmgEffectTimer == 0) && (player->invincibilityTimer == 0) &&
            !(player->stateFlags2 & 0x80) && ((this->collider1.base.ocFlags1 & 2) || (this->collider2.base.ocFlags1 & 2))) {
            this->collider1.base.ocFlags1 &= ~2;
            this->collider2.base.ocFlags1 &= ~2;
            // catch
...
void EnRr_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    this->collider2.shape.pos.x = this->mouthPos.x;
    this->collider2.shape.pos.y = this->mouthPos.y;
    this->collider2.shape.pos.z = this->mouthPos.z;
