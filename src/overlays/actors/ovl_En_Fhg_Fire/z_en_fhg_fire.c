...
extern ColliderCylinderSrc D_80A11790;
...
void EnFhgFire_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((thisx->params == 0x23) || (thisx->params == 0x24) || (thisx->params == 0x32)) {
        ...
        Collider_LoadCylinder(globalCtx, &this->collider, thisx, &D_80A11790);
    }
    this->unk_160 = 200.0f;
    ...
    if (thisx->params == 0x24) {
        ...
        this->collider.shape.radius = tempf1;
        this->collider.shape.height = tempf1;
    ...
    if (thisx->params == 0x32) {
        ...
        this->collider.shape.radius = 40;
        this->collider.shape.height = 50;
        this->collider.shape.yShift = -25;
...
void func_80A0FD8C(EnFhgFire* this, GlobalContext* globalCtx) {
    ...
    if (3.0f < this->unk_18C) {
        Collider_UpdateCylinderShape(&this->actor, &this->collider);
        if (player->invincibilityTimer == 0) {
            Collider_AddAT(globalCtx, &globalCtx->colliderCtx, (Collider*)&this->collider);
        }
    }
