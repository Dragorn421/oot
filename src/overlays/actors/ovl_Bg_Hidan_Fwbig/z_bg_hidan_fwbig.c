...
void BgHidanFwbig_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (this->direction != 0) {
        ...
        this->collider.shape.height = 230;
...
void BgHidanFwbig_MoveCollider(BgHidanFwbig* this, GlobalContext* globalCtx) {
    ...
    this->collider.shape.pos.x = this->actor.posRot.pos.x + (projPos.x * cs) + (projPos.z * sn);
    this->collider.shape.pos.z = this->actor.posRot.pos.z - (projPos.x * sn) + (projPos.z * cs);
