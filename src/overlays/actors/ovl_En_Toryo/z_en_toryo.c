...
static ColliderCylinderSrc sColliderCylinderSrc = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
        AC_NONE,
...
static CollideDataInitAlt sCollideDataInitAlt = { 0x00, 0x00, 0x00, 0x00, 0xFF };
...
void EnToryo_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sColliderCylinderSrc);
    func_80061EFC(&this->actor.collideData, &sActorDamageChart, &sCollideDataInitAlt);
...
void EnToryo_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, (Collider*)collider);
