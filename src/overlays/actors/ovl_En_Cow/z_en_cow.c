...
void func_809DEE9C(EnCow* this) {
    ...
    this->colliders[0].shape.pos.x = this->actor.posRot.pos.x + vec.x;
    this->colliders[0].shape.pos.y = this->actor.posRot.pos.y;
    this->colliders[0].shape.pos.z = this->actor.posRot.pos.z + vec.z;

    vec.x = 0.0f;
    vec.y = 0.0f;
    vec.z = -20.0f;
    func_809DEE00(&vec, this->actor.shape.rot.y);
    this->colliders[1].shape.pos.x = this->actor.posRot.pos.x + vec.x;
    this->colliders[1].shape.pos.y = this->actor.posRot.pos.y;
    this->colliders[1].shape.pos.z = this->actor.posRot.pos.z + vec.z;
}
...
void EnCow_Init(EnCow* this, GlobalContext* globalCtx) {
    ...
    switch (this->actor.params) {
        case 0:
            ...
            Collider_LoadCylinder(globalCtx, &this->colliders[0], &this->actor, &cylinderInit);
            Collider_InitCylinder(globalCtx, &this->colliders[1]);
            Collider_LoadCylinder(globalCtx, &this->colliders[1], &this->actor, &cylinderInit);
            func_809DEE9C(this);
...
void EnCow_Update(EnCow* this, GlobalContext* globalCtx) {
    s32 pad;
    ColliderContext* colChckCtx = &globalCtx->colliderCtx;
    s16 targetX;
    s16 targetY;
    Player* player = PLAYER;

    Collider_AddOC(globalCtx, colChckCtx, &this->colliders[0].base);
    if (globalCtx) {} // necessary to match
    Collider_AddOC(globalCtx, colChckCtx, &this->colliders[1].base);
