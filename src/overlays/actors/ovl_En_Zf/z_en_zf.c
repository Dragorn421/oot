...
typedef enum EnZfDamageReaction {
    /* 0x0 */ ENZF_DMG_REACTION_NONE,
    /* 0x1 */ ENZF_DMG_REACTION_STUN,
    /* 0x6 */ ENZF_DMG_REACTION_IMMUNE = 6,       // Skips damage code, but also skips the top half of Update
    /* 0xD */ ENZF_DMG_REACTION_PROJECTILE = 0xD, // Projectiles that don't have another damageeffect
    /* 0xF */ ENZF_DMG_REACTION_ICE = 0xF
} EnZfDamageReaction;

static DamageTable sDamageTable = {
    /* Deku nut      */ DMG_ENTRY(0, ENZF_DMG_REACTION_STUN),
    /* Deku stick    */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Slingshot     */ DMG_ENTRY(1, ENZF_DMG_REACTION_PROJECTILE),
    /* Explosive     */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Boomerang     */ DMG_ENTRY(0, ENZF_DMG_REACTION_STUN),
    /* Normal arrow  */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Hammer swing  */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Hookshot      */ DMG_ENTRY(0, ENZF_DMG_REACTION_STUN),
    /* Kokiri sword  */ DMG_ENTRY(1, ENZF_DMG_REACTION_NONE),
    /* Master sword  */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Giant's Knife */ DMG_ENTRY(4, ENZF_DMG_REACTION_NONE),
    /* Fire arrow    */ DMG_ENTRY(2, ENZF_DMG_REACTION_PROJECTILE),
    /* Ice arrow     */ DMG_ENTRY(4, ENZF_DMG_REACTION_ICE),
    /* Light arrow   */ DMG_ENTRY(2, ENZF_DMG_REACTION_PROJECTILE),
    /* Unk arrow 1   */ DMG_ENTRY(2, ENZF_DMG_REACTION_PROJECTILE),
    /* Unk arrow 2   */ DMG_ENTRY(2, ENZF_DMG_REACTION_PROJECTILE),
    /* Unk arrow 3   */ DMG_ENTRY(2, ENZF_DMG_REACTION_PROJECTILE),
    /* Fire magic    */ DMG_ENTRY(0, ENZF_DMG_REACTION_IMMUNE),
    /* Ice magic     */ DMG_ENTRY(3, ENZF_DMG_REACTION_ICE),
    /* Light magic   */ DMG_ENTRY(0, ENZF_DMG_REACTION_IMMUNE),
    /* Shield        */ DMG_ENTRY(0, ENZF_DMG_REACTION_NONE),
    /* Mirror Ray    */ DMG_ENTRY(0, ENZF_DMG_REACTION_NONE),
    /* Kokiri spin   */ DMG_ENTRY(1, ENZF_DMG_REACTION_NONE),
    /* Giant spin    */ DMG_ENTRY(4, ENZF_DMG_REACTION_NONE),
    /* Master spin   */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Kokiri jump   */ DMG_ENTRY(2, ENZF_DMG_REACTION_NONE),
    /* Giant jump    */ DMG_ENTRY(8, ENZF_DMG_REACTION_NONE),
    /* Master jump   */ DMG_ENTRY(4, ENZF_DMG_REACTION_NONE),
    /* Unknown 1     */ DMG_ENTRY(0, ENZF_DMG_REACTION_NONE),
    /* Unblockable   */ DMG_ENTRY(0, ENZF_DMG_REACTION_NONE),
    /* Hammer jump   */ DMG_ENTRY(4, ENZF_DMG_REACTION_NONE),
    /* Unknown 2     */ DMG_ENTRY(0, ENZF_DMG_REACTION_NONE),
};
...
void EnZf_SetupStunned(EnZf* this) {
    ...
    if (this->damageReaction == ENZF_DMG_REACTION_ICE) {
        this->iceTimer = 36;
    } else {
...
void EnZf_Damaged(EnZf* this, PlayState* play) {
    ...
    if (((this->actor.params != ENZF_TYPE_DINOLFOS) || !EnZf_ChooseAction(play, this)) &&
        ...
        if (D_80B4A1B4 != -1) {
            if (this->damageReaction == ENZF_DMG_REACTION_PROJECTILE) {
                D_80B4A1B0++;
            } else {
                this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnZf_UpdateDamage(EnZf* this, PlayState* play) {
    ...
    if ((this->bodyCollider.base.acFlags & AC_HIT) && (this->action <= ENZF_ACTION_STUNNED)) {
        ...
        if (((this->actor.params < ENZF_TYPE_LIZALFOS_MINIBOSS_A) /* not miniboss */ ||
             (D_80B4A1B4 != this->actor.params)) &&
            (this->actor.colChkInfo.damageReaction != ENZF_DMG_REACTION_IMMUNE)) {
            this->damageReaction = this->actor.colChkInfo.damageReaction;
            Actor_SetDropFlag(&this->actor, &this->bodyCollider.elem, false);

            if ((this->actor.colChkInfo.damageReaction == ENZF_DMG_REACTION_STUN) ||
                (this->actor.colChkInfo.damageReaction == ENZF_DMG_REACTION_ICE)) {
                ...
                if (Actor_ApplyDamage(&this->actor) == 0) {
                    ...
                    if ((D_80B4A1B4 != -1) && ((this->actor.colChkInfo.health + this->actor.colChkInfo.damage) >= 4) &&
                        ...
                        this->damageReaction = ENZF_DMG_REACTION_PROJECTILE;
                    }
...
void EnZf_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.colChkInfo.damageReaction != ENZF_DMG_REACTION_IMMUNE) {
        this->unk_3F8 = false;
