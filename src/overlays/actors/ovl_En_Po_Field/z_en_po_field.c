...
static ColliderCylinderSrc D_80AD7080 = {
    {
        COL_MATERIAL_HIT3,
        AT_NONE,
...
static ColliderCylinderSrc D_80AD70AC = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
static CollideDataInit D_80AD70D8 = { 0x04, 0x0019, 0x0032, 0x28 };
...
void EnPoField_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &D_80AD7080);
    Collider_InitCylinder(globalCtx, &this->flameCollider);
    Collider_LoadCylinder(globalCtx, &this->flameCollider, &this->actor, &D_80AD70AC);
    func_80061ED4(&this->actor.collideData, &sActorDamageChart, &D_80AD70D8);
...
void EnPoField_SetupWaitForSpawn(EnPoField* this, GlobalContext* globalCtx) {
    ...
    this->collider.base.ocFlags1 = 0x39;
    this->actor.collideData.health = D_80AD70D8.health;
...
void EnPoField_SetupAppear(EnPoField* this) {
    ...
    if (this->actor.params == EN_PO_FIELD_BIG) {
        ...
        this->collider.shape.radius = 35;
        this->collider.shape.height = 100;
        this->collider.shape.yShift = 10;
        this->actor.shape.unk_10 = 45.0f;
        this->scaleModifier = 0.014f;
        this->actor.naviEnemyId = 0x5A;
    } else {
        this->actor.speedXZ = 0.0f;
        this->collider.shape.radius = D_80AD7080.shape.radius;
        this->collider.shape.height = D_80AD7080.shape.height;
        this->collider.shape.yShift = D_80AD7080.shape.yShift;
...
void EnPoField_SetupDamage(EnPoField* this) {
    ...
    if (this->collider.elem.acHitElem->atDmgInfo.dmgFlags & 0x1F824) {
...
void func_80AD4384(EnPoField* this) {
    ...
    this->collider.shape.radius = 13;
    this->collider.shape.height = 30;
    this->collider.shape.yShift = 0;
    this->collider.shape.pos.x = this->actor.posRot.pos.x;
    this->collider.shape.pos.y = this->actor.posRot.pos.y - 20.0f;
    this->collider.shape.pos.z = this->actor.posRot.pos.z;
...
void EnPoField_UpdateFlame(EnPoField* this, GlobalContext* globalCtx) {
    if (this->flameTimer != 0) {
        ...
        this->flameCollider.shape.pos.x = this->flamePosition.x;
        this->flameCollider.shape.pos.y = this->flamePosition.y;
        this->flameCollider.shape.pos.z = this->flamePosition.z;
        Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->flameCollider.base);
    }
}
