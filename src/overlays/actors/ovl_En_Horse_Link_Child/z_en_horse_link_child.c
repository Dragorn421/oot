...
static ColliderSpheresElementSrc sJntSphItemInit[1] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
static CollideDataInit sColCheckInfoInit = { 10, 35, 100, 0xFE };
...
void EnHorseLinkChild_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinderAlt(globalCtx, &this->bodyCollider, &this->actor, &sCylinderInit);
    Collider_InitSpheres(globalCtx, &this->headCollider);
    Collider_LoadSpheres(globalCtx, &this->headCollider, &this->actor, &sJntSphInit, this->headColliderItems);
    func_80061ED4(&this->actor.collideData, NULL, &sColCheckInfoInit);
    this->unk_1F0 = 0;
    this->unk_1EC = 0;
...
void EnHorseLinkChild_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_DestroySpheres(globalCtx, &this->headCollider);
}
...
void func_80A6ABF8(Actor* thisx, GlobalContext* globalCtx, ColliderSpheresElement* collider) {
    ...
    for (i = 0; i < this->headCollider.nElements; i++) {
        ...
        this->headCollider.elements[i].shape.world.radius =
            this->headCollider.elements[i].shape.model.radius * this->headCollider.elements[i].shape.unkRadiusScale;
    }

    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->headCollider.base);
}
