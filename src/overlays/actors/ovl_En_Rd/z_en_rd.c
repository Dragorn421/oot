...
ColliderCylinderSrc sCylinderInit_EnRd = {
    {
        COL_MATERIAL_HIT0,
        AT_NONE,
...
void EnRd_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    thisx->collideData.mass = 0xFE;
    thisx->collideData.health = 8;
    ...
    Collider_LoadCylinder(globalCtx, &this->collider, thisx, &sCylinderInit_EnRd);
...
void func_80AE4114(EnRd* this, GlobalContext* globalCtx) {
    ...
    if (this->collider.base.acFlags & 2) {
        ...
        this->unk_31C = this->actor.collideData.damageEffect;

        if (this->unk_31B != 11) {
            func_80035650(&this->actor, &this->collider.elem, 1);
            if (player->unk_844 != 0) {
...
void EnRd_Update(Actor* thisx, GlobalContext* globalCtx) {
    EnRd* this = THIS;
    ColliderContext* colliderCtx = &globalCtx->colliderCtx;
    Player* player = PLAYER;
    ...
    if ((this->actor.collideData.health > 0) && (this->unk_31B != 8)) {
        Collider_UpdateCylinderShape(&this->actor, collider);
        Collider_AddOC(globalCtx, colliderCtx, &collider->base);
        if ((this->unk_31B != 9) || ((player->unk_844 != 0) && (player->unk_845 != this->unk_31D))) {
            Collider_AddAC(globalCtx, colliderCtx, &collider->base);
        }
    }
}
