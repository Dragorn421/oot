...
static ColliderCylinderSrc sCylinderInitCapturableFlame = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
        AC_NONE,
...
static ColliderCylinderSrc sCylinderInitDroppedFlame = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_OTHER,
        AC_NONE,
...
void EnIceHono_InitCapturableFlame(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInitCapturableFlame);
...
void EnIceHono_InitDroppedFlame(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_InitCylinder(globalCtx, &this->collider);
    Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInitDroppedFlame);
    Collider_UpdateCylinderShape(&this->actor, &this->collider);

    this->collider.shape.radius = this->actor.scale.x * 4000.4f;
    this->collider.shape.height = this->actor.scale.y * 8000.2f;
    this->actor.collideData.mass = 253;
...
void EnIceHono_DropFlame(EnIceHono* this, GlobalContext* globalCtx) {
    ...
    Collider_UpdateCylinderShape(&this->actor, &this->collider);
    this->collider.shape.radius = this->actor.scale.x * 4000.0f;
    this->collider.shape.height = this->actor.scale.y * 8000.0f;
    Collider_AddOC(globalCtx, &globalCtx->colliderCtx, &this->collider.base);
...
void EnIceHono_SpreadFlames(EnIceHono* this, GlobalContext* globalCtx) {
    ...
    if ((this->alpha > 100) && (this->timer < 40)) {
        Collider_UpdateCylinderShape(&this->actor, &this->collider);
        this->collider.shape.radius = this->actor.scale.x * 6000.0f;
        this->collider.shape.height = this->actor.scale.y * 8000.0f;
