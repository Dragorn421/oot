...
void EnWonderItem_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (this->wonderMode) {
        ...
        case WONDERITEM_INTERACT_SWITCH:
            ...
            this->collider.elem.acDmgInfo.dmgFlags = collisionTypes[colTypeIndex];
            this->collider.shape.radius = 20;
        ...
        case WONDERITEM_BOMB_SOLDIER:
            Collider_InitCylinder(globalCtx, &this->collider);
            Collider_LoadCylinder(globalCtx, &this->collider, &this->actor, &sCylinderInit);
            this->collider.elem.acDmgInfo.dmgFlags = 0x00000004; // slingshot
            this->unkPos = this->actor.posRot.pos;
            this->collider.shape.radius = 35;
            this->collider.shape.height = 75;
