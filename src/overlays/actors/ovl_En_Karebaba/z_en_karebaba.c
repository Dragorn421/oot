...
static ColliderCylinderSrc sBodyColliderInit = {
    {
        COL_MATERIAL_HARD,
        AT_NONE,
...
static ColliderCylinderSrc sHeadColliderInit = {
    {
        COL_MATERIAL_HARD,
        AT_ON | AT_TYPE_ENEMY,
        AC_NONE,
...
static CollideDataInit sColCheckInfoInit = { 1, 15, 80, 0xFE };
...
void EnKarebaba_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->bodyCollider, &this->actor, &sBodyColliderInit);
    Collider_UpdateCylinderShape(&this->actor, &this->bodyCollider);
    Collider_InitCylinder(globalCtx, &this->headCollider);
    Collider_LoadCylinder(globalCtx, &this->headCollider, &this->actor, &sHeadColliderInit);
    Collider_UpdateCylinderShape(&this->actor, &this->headCollider);
    func_80061ED4(&this->actor.collideData, CollisionBtlTbl_Get(1), &sColCheckInfoInit);
...
void EnKarebaba_ResetCollider(EnKarebaba* this) {
    this->bodyCollider.shape.radius = 7;
    this->bodyCollider.shape.height = 25;
    this->bodyCollider.base.colMaterial = COL_MATERIAL_HARD;
    this->bodyCollider.base.acFlags |= 4;
    this->bodyCollider.elem.acDmgInfo.dmgFlags = ~0x00300000;
    this->headCollider.shape.height = 25;
}
...
void EnKarebaba_SetupUpright(EnKarebaba* this) {
    if (this->actionFunc != EnKarebaba_Spin) {
        ...
        this->bodyCollider.elem.acDmgInfo.dmgFlags = gSaveContext.linkAge != 0 ? 0x07C00710 : 0x0FC00710;
        this->bodyCollider.shape.radius = 15;
        this->bodyCollider.shape.height = 80;
        this->headCollider.shape.height = 80;
    }
...
void EnKarebaba_SetupRegrow(EnKarebaba* this) {
    ...
    this->headCollider.shape.radius = sHeadColliderInit.shape.radius;
    Actor_SetScale(&this->actor, 0.0f);
...
void EnKarebaba_Spin(EnKarebaba* this, GlobalContext* globalCtx) {
    ...
    this->headCollider.shape.radius = sHeadColliderInit.shape.radius + (value * 2);
    ...
    if (this->actionFunc != EnKarebaba_Dead) {
        ...
        if (this->actionFunc != EnKarebaba_Dying && this->actionFunc != EnKarebaba_DeadItemDrop) {
            if (this->actionFunc != EnKarebaba_Regrow && this->actionFunc != EnKarebaba_Grow) {
                Collider_AddAT(globalCtx, &globalCtx->colliderCtx, &this->headCollider.base);
