...
void DoorWarp1_ChildWarpOut(DoorWarp1* this, PlayState* play) {
    ...
    if (sWarpTimerTarget < this->warpTimer && GET_NEXTCUTSCENEINDEX == 0xFFEF) {
...
void DoorWarp1_RutoWarpOut(DoorWarp1* this, PlayState* play) {
    ...
    if (this->warpTimer > sWarpTimerTarget && GET_NEXTCUTSCENEINDEX == 0xFFEF) {
    ...
    if (this->warpTimer > sWarpTimerTarget && GET_NEXTCUTSCENEINDEX == 0xFFEF) {
        if (play->sceneId == SCENE_MORIBOSSROOM) {
            if (!GET_EVENTCHKINF(EVENTCHKINF_48)) {
                ...
                SET_NEXTCUTSCENEINDEX(0);
                SET_CHAMBERCUTSCENENUM(CHAMBER_CS_FOREST);
            } else {
                if (!LINK_IS_ADULT) {
            ...
            if (!GET_EVENTCHKINF(EVENTCHKINF_4A)) {
                ...
                play->nextEntranceIndex = ENTR_KENJYANOMA_0;
                SET_NEXTCUTSCENEINDEX(0);
                SET_CHAMBERCUTSCENENUM(CHAMBER_CS_WATER);
            } else {
                if (!LINK_IS_ADULT) {
            ...
            if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_SPIRIT)) {
                ...
                play->nextEntranceIndex = ENTR_KENJYANOMA_0;
                SET_NEXTCUTSCENEINDEX(0);
                SET_CHAMBERCUTSCENENUM(CHAMBER_CS_SPIRIT);
            } else {
                if (!LINK_IS_ADULT) {
            ...
            if (!CHECK_QUEST_ITEM(QUEST_MEDALLION_SHADOW)) {
                ...
                play->nextEntranceIndex = ENTR_KENJYANOMA_0;
                SET_NEXTCUTSCENEINDEX(0);
                SET_CHAMBERCUTSCENENUM(CHAMBER_CS_SHADOW);
            } else {
                if (!LINK_IS_ADULT) {
        ...
        SET_NEXTTRANSITIONTYPE(TRANS_TYPE_FADE_WHITE_SLOW);
    }
