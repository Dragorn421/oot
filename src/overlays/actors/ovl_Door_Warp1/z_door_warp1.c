...
void DoorWarp1_ChildWarpOut(DoorWarp1* this, PlayState* play) {
    ...
    if (sWarpTimerTarget < this->warpTimer && gSaveContext.nextCutsceneIndex == NEXT_CUTSCENE_INDEX_FFEF) {
        ...
        if (play->sceneId == SCENE_DODONGOS_CAVERN_BOSS) {
            if (!Flags_GetEventChkInf(EVENTCHKINF_25)) {
                ...
                gSaveContext.nextCutsceneIndex = CUTSCENE_INDEX_0;
            }
...
void DoorWarp1_RutoWarpOut(DoorWarp1* this, PlayState* play) {
    ...
    if (this->warpTimer > sWarpTimerTarget && gSaveContext.nextCutsceneIndex == NEXT_CUTSCENE_INDEX_FFEF) {
    ...
    if (this->warpTimer > sWarpTimerTarget && gSaveContext.nextCutsceneIndex == NEXT_CUTSCENE_INDEX_FFEF) {
        if (play->sceneId == SCENE_FOREST_TEMPLE_BOSS) {
            ...
            if (!GET_EVENTCHKINF(EVENTCHKINF_49)) {
                ...
                gSaveContext.nextCutsceneIndex = CUTSCENE_INDEX(3);
            } else {
                if (!LINK_IS_ADULT) {
