...
ColliderCylinderSrc sCylinderInit_EnBombf = {
    {
        COL_MATERIAL_NONE,
        AT_NONE,
...
ColliderSpheresElementSrc sJntSphItemsInit_EnBombf[1] = {
    {
        {
            ELEM_MATERIAL_UNK0,
            {
...
ColliderSpheresSrc sJntSphInit_EnBombf = {
    {
        COL_MATERIAL_NONE,
        AT_ON | AT_TYPE_ALL,
        AC_NONE,
...
void EnBombf_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    Collider_LoadCylinder(globalCtx, &this->bombCollider, &this->actor, &sCylinderInit_EnBombf);
    Collider_LoadSpheres(globalCtx, &this->explosionCollider, &this->actor, &sJntSphInit_EnBombf,
    ...
    this->actor.collideData.unk_10 = 10.0f;
    this->actor.collideData.unk_12 = 10;
    this->actor.unk_1F = 0;
...
void EnBombf_Explode(EnBombf* this, GlobalContext* globalCtx) {
    ...
    this->explosionCollider.elements[0].shape.model.radius += 8;
    this->explosionCollider.elements[0].shape.world.radius = this->explosionCollider.elements[0].shape.model.radius;
...
void EnBombf_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params <= BOMBFLOWER_BODY) {
        ...
        Collider_UpdateCylinderShape(&this->actor, &this->bombCollider); // bombCollider goes to sp38 instead of sp3C

        if ((this->flowerBombScale >= 1.0f) && (this->bumpOn)) {
            Collider_AddOC(globalCtx, &globalCtx->colliderCtx,
                                 &this->bombCollider.base); // bombCollider loads from sp38 instead of sp3C
        }

        Collider_AddAC(globalCtx, &globalCtx->colliderCtx,
                             &this->bombCollider.base); // bombCollider loads from sp38 instead of sp3C
    }
