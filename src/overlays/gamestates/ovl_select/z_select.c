...
void MapSelect_LoadGame(MapSelectState* this, s32 entranceIndex) {
    ...
    SET_ENTRANCEINDEX(entranceIndex);
    SET_RESPAWNFLAG(0);
    SET_RESPAWN_A0_ENTRANCEINDEX(RESPAWN_MODE_DOWN, ENTR_LOAD_OPENING);
...
void MapSelect_UpdateMenu(MapSelectState* this) {
    ...
    if (this->verticalInputAccumulator == 0) {
        ...
        if (CHECK_BTN_ALL(input->press.button, BTN_Z)) {
            if (GET_CUTSCENEINDEX == 0x8000) {
                SET_CUTSCENEINDEX(0);
            } else if (GET_CUTSCENEINDEX == 0) {
                SET_CUTSCENEINDEX(0xFFF0);
            } else if (GET_CUTSCENEINDEX == 0xFFF0) {
                SET_CUTSCENEINDEX(0xFFF1);
            } else if (GET_CUTSCENEINDEX == 0xFFF1) {
                SET_CUTSCENEINDEX(0xFFF2);
            } else if (GET_CUTSCENEINDEX == 0xFFF2) {
                SET_CUTSCENEINDEX(0xFFF3);
            } else if (GET_CUTSCENEINDEX == 0xFFF3) {
                SET_CUTSCENEINDEX(0xFFF4);
            } else if (GET_CUTSCENEINDEX == 0xFFF4) {
                SET_CUTSCENEINDEX(0xFFF5);
            } else if (GET_CUTSCENEINDEX == 0xFFF5) {
                SET_CUTSCENEINDEX(0xFFF6);
            } else if (GET_CUTSCENEINDEX == 0xFFF6) {
                SET_CUTSCENEINDEX(0xFFF7);
            } else if (GET_CUTSCENEINDEX == 0xFFF7) {
                SET_CUTSCENEINDEX(0xFFF8);
            } else if (GET_CUTSCENEINDEX == 0xFFF8) {
                SET_CUTSCENEINDEX(0xFFF9);
            } else if (GET_CUTSCENEINDEX == 0xFFF9) {
                SET_CUTSCENEINDEX(0xFFFA);
            } else if (GET_CUTSCENEINDEX == 0xFFFA) {
...
void MapSelect_PrintCutsceneSetting(MapSelectState* this, GfxPrint* printer, u16 csIndex) {
    ...
    switch (csIndex) {
        case 0:
            ...
            SET_DAYTIME(CLOCK_TIME(0, 0));
            break;
        case 0x8000:
            // clang-format off
            SET_DAYTIME(CLOCK_TIME(12, 0)); label = GFXP_HIRAGANA "ｵﾋﾙ " GFXP_KATAKANA "ｼﾞｬﾗ";
            // clang-format on
            break;
        case 0xFFF0:
            // clang-format off
            SET_DAYTIME(CLOCK_TIME(12, 0)); label = "ﾃﾞﾓ00";
            // clang-format on
            break;
        case 0xFFF1:
            label = "ﾃﾞﾓ01";
...
void MapSelect_DrawMenu(MapSelectState* this) {
    ...
    MapSelect_PrintAgeSetting(this, printer, (GET_LINKAGE_VOID0));
    MapSelect_PrintCutsceneSetting(this, printer, (GET_CUTSCENEINDEX_VOID0));
