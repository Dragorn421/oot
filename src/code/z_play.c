...
void Play_Init(GameState* thisx) {
    ...
    PRINTF(_2("ゼルダヒープ", "Zelda Heap") " %08x-%08x\n", zAllocAligned,
                ...
                case TRANS_MODE_SETUP:
                    if (this->transitionTrigger != TRANS_TRIGGER_END) {
                        ...
                        if (!(gEntranceTable[this->nextEntranceIndex + sceneLayer].field &
                            ...
                            PRINTF(_2("\n\n\nサウンドイニシャル来ました。111", "Sound initialized. 111"));
                            if ((this->transitionType < TRANS_TYPE_MAX) && !Environment_IsForcedSequenceDisabled()) {
                                PRINTF(_2("\n\n\nサウンドイニシャル来ました。222", "Sound initialized. 222"));
                                func_800F6964(0x14);
            ...
            if (this->viewpoint != VIEWPOINT_NONE) {
                if (CHECK_BTN_ALL(input[0].press.button, BTN_CUP)) {
                    if (IS_PAUSED(&this->pauseCtx)) {
                        PRINTF(VT_FGCOL(CYAN) _2("カレイドスコープ中につき視点変更を禁止しております\n",
                                                 "Changing viewpoint is prohibited due to the kaleidoscope\n") VT_RST);
                    } else if (Player_InCsMode(this)) {
                        PRINTF(VT_FGCOL(CYAN) _2("デモ中につき視点変更を禁止しております\n",
