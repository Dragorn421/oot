...
void Play_SetViewpoint(PlayState* this, s16 viewpoint) {
    ...
    if ((R_SCENE_CAM_TYPE != SCENE_CAM_TYPE_FIXED_SHOP_VIEWPOINT) && (GET_CUTSCENE_INDEX < 0xFFF0)) {
...
void Play_Destroy(GameState* thisx) {
    ...
    if (GET_LINK_AGE != this->linkAgeOnLoad) {
...
void Play_Init(GameState* thisx) {
    ...
    if (GET_ENTRANCE_INDEX == ENTR_LOAD_OPENING) {
        SET_ENTRANCE_INDEX(0)
    ...
    if (GET_NEXT_CUTSCENE_INDEX != 0xFFEF) {
        SET_CUTSCENE_INDEX(gSaveContext.nextCutsceneIndex)
        SET_NEXT_CUTSCENE_INDEX(0xFFEF)
    }

    if (GET_CUTSCENE_INDEX == 0xFFFD) {
        SET_CUTSCENE_INDEX(0)
    }

    if (GET_NEXT_DAY_TIME != NEXT_TIME_NONE) {
        SET_DAY_TIME(gSaveContext.nextDayTime)
        SET_SKYBOX_TIME(gSaveContext.nextDayTime)
    }

    if (GET_DAY_TIME > CLOCK_TIME(18, 0) || GET_DAY_TIME < CLOCK_TIME(6, 30)) {
        SET_NIGHT_FLAG(1)
    } else {
        SET_NIGHT_FLAG(0)
    }
    ...
    if (GET_GAME_MODE != GAMEMODE_NORMAL || GET_CUTSCENE_INDEX >= 0xFFF0) {
        SET_NAYRUS_LOVE_TIMER(0)
        Magic_Reset(this);
        SET_SCENE_LAYER(SCENE_LAYER_CUTSCENE_FIRST + (gSaveContext.save.cutsceneIndex & 0xF))
    } else if (!LINK_IS_ADULT && IS_DAY) {
        SET_SCENE_LAYER(SCENE_LAYER_CHILD_DAY)
    } else if (!LINK_IS_ADULT && !IS_DAY) {
        SET_SCENE_LAYER(SCENE_LAYER_CHILD_NIGHT)
    } else if (LINK_IS_ADULT && IS_DAY) {
        SET_SCENE_LAYER(SCENE_LAYER_ADULT_DAY)
    } else {
        SET_SCENE_LAYER(SCENE_LAYER_ADULT_NIGHT)
    }

    // save the base scene layer (before accounting for the special cases below) to use later for the transition type
    baseSceneLayer = GET_SCENE_LAYER;

    if ((gEntranceTable[(GET_ENTRANCE_INDEX_VOID0)].sceneId == SCENE_SPOT00) && !LINK_IS_ADULT &&
        !IS_CUTSCENE_LAYER) {
    ...
    } else if ((gEntranceTable[(GET_ENTRANCE_INDEX_VOID0)].sceneId == SCENE_SPOT04) && LINK_IS_ADULT &&
               !IS_CUTSCENE_LAYER) {
        SET_SCENE_LAYER(GET_EVENTCHKINF(EVENTCHKINF_48) ? 3 : 2)
    }

    Play_SpawnScene(
        this, gEntranceTable[(GET_ENTRANCE_INDEX_VOID0) + (GET_SCENE_LAYER_VOID0)].sceneId,
        gEntranceTable[(GET_ENTRANCE_INDEX_VOID0) + (GET_SCENE_LAYER_VOID0)].spawn);

    osSyncPrintf("\nSCENE_NO=%d COUNTER=%d\n", (GET_ENTRANCE_INDEX_VOID0), GET_SCENE_LAYER);

    // When entering Gerudo Valley in the credits, trigger the GC emulator to play the ending movie.
    // The emulator constantly checks whether PC is 0x81000000, so this works even though it's not a valid address.
    if ((gEntranceTable[(GET_ENTRANCE_INDEX_VOID0)].sceneId == SCENE_SPOT09) &&
        GET_SCENE_LAYER == 6) {
    ...
    if (GET_NEXT_DAY_TIME != NEXT_TIME_NONE) {
        if (GET_NEXT_DAY_TIME == NEXT_TIME_DAY) {
            GET_TOTAL_DAYS++;
            GET_BGS_DAY_COUNT++;
            ...
            SET_NEXT_DAY_TIME(NEXT_TIME_DAY_SET)
        } else {
            SET_NEXT_DAY_TIME(NEXT_TIME_NIGHT_SET)
        }
    }
    ...
    if (GET_GAME_MODE != GAMEMODE_TITLE_SCREEN) {
        if (GET_NEXT_TRANSITION_TYPE == TRANS_NEXT_TYPE_DEFAULT) {
            this->transitionType = ENTRANCE_INFO_END_TRANS_TYPE(
                gEntranceTable[(GET_ENTRANCE_INDEX_VOID0) + baseSceneLayer].field);
        } else {
            this->transitionType = GET_NEXT_TRANSITION_TYPE;
    ...
    SET_SEQ_ID(this->sequenceCtx.seqId)
    SET_NATURE_AMBIENCE_ID(this->sequenceCtx.natureAmbienceId)
...
void Play_Update(PlayState* this) {
    ...
    if (FrameAdvance_Update(&this->frameAdvCtx, &input[1])) {
        ...
        if (this->transitionMode) { // != TRANS_MODE_OFF
            switch (this->transitionMode) {
                case TRANS_MODE_SETUP:
                    if (this->transitionTrigger != TRANS_TRIGGER_END) {
                        ...
                        if (GET_CUTSCENE_INDEX >= 0xFFF0) {
                            sceneLayer = SCENE_LAYER_CUTSCENE_FIRST + (GET_CUTSCENE_INDEX & 0xF);
                        }
                ...
                case TRANS_MODE_INSTANCE_INIT:
                    ...
                    if ((this->transitionCtx.transitionType == TRANS_TYPE_WIPE_FAST) ||
                        ...
                        SET_TRANS_WIPE_SPEED(28)
                    }

                    SET_TRANS_FADE_DURATION(60)
                ...
                case TRANS_MODE_INSTANCE_RUNNING:
                    if (this->transitionCtx.isDone(&this->transitionCtx.instanceData)) {
                        if (this->transitionCtx.transitionType >= TRANS_TYPE_MAX) {
                            ...
                            if (GET_GAME_MODE != GAMEMODE_FILE_SELECT) {
                                SET_NEXT_GAMESTATE(&this->state, Play_Init, PlayState);
                                SET_ENTRANCE_INDEX(this->nextEntranceIndex)

                                if (GET_MINIGAME_STATE == 1) {
                                    SET_MINIGAME_STATE(3)
                                }
                            } else {
            ...
            switch (this->transitionMode) {
                ...
                case TRANS_MODE_INSTANCE_WAIT:
                    if (GET_CUTSCENE_TRANSITION_CONTROL != 0) {
                        this->transitionMode = TRANS_MODE_INSTANCE_RUNNING;
                    }
                    break;
                ...
                case TRANS_MODE_CS_BLACK_FILL:
                    if (GET_CUTSCENE_TRANSITION_CONTROL != 0) {
                        this->envCtx.screenFillColor[3] = GET_CUTSCENE_TRANSITION_CONTROL;

                        if (GET_CUTSCENE_TRANSITION_CONTROL <= 100) {
                            gTrnsnUnkState = 0;
                            R_UPDATE_RATE = 3;
                            this->transitionTrigger = TRANS_TRIGGER_OFF;
                            this->transitionMode = TRANS_MODE_OFF;
        ...
        if (1 && (gTrnsnUnkState != 3)) {
            ...
            if ((GET_GAME_MODE == GAMEMODE_NORMAL) && (this->msgCtx.msgMode == MSGMODE_NONE) &&
...
void Play_DrawOverlayElements(PlayState* this) {
    ...
    if (GET_GAME_MODE == GAMEMODE_NORMAL) {
        Interface_Draw(this);
    }
...
void Play_SaveSceneFlags(PlayState* this) {
    SavedSceneFlags* savedSceneFlags = &GET_SCENE_FLAGS[this->sceneId];
...
void Play_SetRespawnData(PlayState* this, s32 respawnMode, s16 entranceIndex, s32 roomIndex, s32 playerParams,
                         Vec3f* pos, s16 yaw) {
    RespawnData* respawnData = &GET_RESPAWN[respawnMode];
...
void Play_SetupRespawnPoint(PlayState* this, s32 respawnMode, s32 playerParams) {
    ...
    if ((this->sceneId != SCENE_YOUSEI_IZUMI_TATE) && (this->sceneId != SCENE_KAKUSIANA)) {
        ...
        entranceIndex = GET_ENTRANCE_INDEX;
...
void Play_TriggerVoidOut(PlayState* this) {
    GET_RESPAWN[RESPAWN_MODE_DOWN].tempSwchFlags = this->actorCtx.flags.tempSwch;
    GET_RESPAWN[RESPAWN_MODE_DOWN].tempCollectFlags = this->actorCtx.flags.tempCollect;
    SET_RESPAWN_FLAG(1)
    this->transitionTrigger = TRANS_TRIGGER_START;
    this->nextEntranceIndex = GET_RESPAWN[RESPAWN_MODE_DOWN].entranceIndex;
...
void Play_LoadToLastEntrance(PlayState* this) {
    ...
    } else if ((GET_ENTRANCE_INDEX == ENTR_SPOT00_11) ||
               (GET_ENTRANCE_INDEX == ENTR_SPOT00_12) ||
               (GET_ENTRANCE_INDEX == ENTR_SPOT00_13) ||
               (GET_ENTRANCE_INDEX == ENTR_SPOT00_15)) {
        this->nextEntranceIndex = ENTR_SPOT00_6;
    } else {
        this->nextEntranceIndex = GET_ENTRANCE_INDEX;
    }
