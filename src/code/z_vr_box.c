...
void Skybox_Setup(PlayState* play, SkyboxContext* skyboxCtx, s16 skyboxId) {
    ...
    switch (skyboxId) {
        case SKYBOX_NORMAL_SKY:
            skyboxConfig = 0;
            if (GET_RETAIN_WEATHER_MODE && !IS_CUTSCENE_LAYER && gWeatherMode > WEATHER_MODE_CLEAR &&
                gWeatherMode <= WEATHER_MODE_HEAVY_RAIN) {
                skyboxConfig = 1;
            }

            for (i = 0; i < ARRAY_COUNT(gTimeBasedSkyboxConfigs[skyboxConfig]); i++) {
                if (GET_SKYBOX_TIME >= gTimeBasedSkyboxConfigs[skyboxConfig][i].startTime &&
                    (GET_SKYBOX_TIME < gTimeBasedSkyboxConfigs[skyboxConfig][i].endTime ||
