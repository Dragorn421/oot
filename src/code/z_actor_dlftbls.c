...
#define DEFINE_ACTOR(name, _1, allocType, nameString)                                            \
    {                                                                                            \
        NULL, "rom:/actors/ovl_" #name ".dso", #name "_Profile", NULL, nameString, allocType, 0, \
    },

#define DEFINE_ACTOR_INTERNAL(name, _1, allocType, nameString)       \
    {                                                                \
        NULL, NULL, NULL, &name##_Profile, nameString, allocType, 0, \
    },
...
void ActorOverlayTable_LogPrint(void) {
    ...
    for (i = 0, overlayEntry = &gActorOverlayTable[0]; i < (u32)gMaxActorId; i++, overlayEntry++) {
        PRINTF("%08x %s\n", overlayEntry->profile, overlayEntry->name != NULL ? overlayEntry->name : "?");
    }
#endif
}
