...
s32 Player_IsChildWithHylianShield(Player* this) {
    return GET_LINK_AGE != LINK_AGE_ADULT && (this->currentShield == PLAYER_SHIELD_HYLIAN);
}
    ...
    if ((this->stateFlags1 & PLAYER_STATE1_22) &&
        ...
        if (!Player_HoldsTwoHandedWeapon(this) && !Player_IsChildWithHylianShield(this)) {
            ...
            this->rightHandDLists =
                sPlayerDListGroups[PLAYER_MODELTYPE_RH_SHIELD] + (GET_LINK_AGE_VOID0);
            ...
            this->sheathDLists = sPlayerDListGroups[this->sheathType] + (GET_LINK_AGE_VOID0);
...
s32 func_8008F2F8(PlayState* play) {
    ...
    if (!Player_InCsMode(play)) {
        ...
        if ((triggerEntry->flag != 0) && !(GET_TEXT_TRIGGER_FLAGS & triggerEntry->flag) &&
            (((var == 0) && (this->currentTunic != PLAYER_TUNIC_GORON)) ||
             (((var == 1) || (var == 3)) && (this->currentBoots == PLAYER_BOOTS_IRON) &&
              (this->currentTunic != PLAYER_TUNIC_ZORA)))) {
            Message_StartTextbox(play, triggerEntry->textId, NULL);
            GET_TEXT_TRIGGER_FLAGS |= triggerEntry->flag;
        }
    }
...
void Player_DrawImpl(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic, s32 boots,
...
#else
    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(sEyeTextures[GET_LINK_AGE][eyeIndex]));
#endif
...
#else
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(sMouthTextures[GET_LINK_AGE][mouthIndex]));
#endif
    ...
    if ((overrideLimbDraw != Player_OverrideLimbDrawGameplayFirstPerson) &&
        ...
        (GET_GAME_MODE != GAMEMODE_END_CREDITS)) {
        if (LINK_IS_ADULT) {
            s32 strengthUpgrade = CUR_UPG_VALUE(UPG_STRENGTH);
...
void func_8008F87C(PlayState* play, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, s32 thighLimbIndex,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & PLAYER_STATE1_7) &&
        ...
        sp7C = D_80126058[GET_LINK_AGE_VOID0];
        sp78 = D_80126060[GET_LINK_AGE_VOID0];
        sp74 = D_80126068[GET_LINK_AGE_VOID0] - this->unk_6C4;

        Matrix_Push();
        Matrix_TranslateRotateZYX(pos, rot);
        Matrix_MultVec3f(&sZeroVec, &spA4);
        Matrix_TranslateRotateZYX(&D_80126038[GET_LINK_AGE_VOID0],
                                  &skelAnime->jointTable[shinLimbIndex]);
        Matrix_Translate(D_80126050[GET_LINK_AGE_VOID0], 0.0f, 0.0f, MTXMODE_APPLY);
...
s32 Player_OverrideLimbDrawGameplayDefault(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            if ((sLeftHandType == PLAYER_MODELTYPE_LH_BGS) && (GET_SWORD_HEALTH <= 0.0f)) {
                dLists += 4;
            } else if ((sLeftHandType == PLAYER_MODELTYPE_LH_BOOMERANG) && (this->stateFlags1 & PLAYER_STATE1_25)) {
                dLists = gPlayerLeftHandOpenDLs + GET_LINK_AGE;
                ...
                dLists = gPlayerLeftHandClosedDLs + GET_LINK_AGE;
            ...
            *dList = *(dLists + sDListsLodOffset);
            ...
            if (sRightHandType == PLAYER_MODELTYPE_RH_SHIELD) {
                dLists += this->currentShield * 4;
                       ...
                       !(this->stateFlags1 & PLAYER_STATE1_27)) {
                dLists = sPlayerRightHandClosedDLs + GET_LINK_AGE;
...
s32 Player_OverrideLimbDrawGameplayFirstPerson(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (this->unk_6AD != 2) {
            ...
            *dList = sFirstPersonLeftForearmDLs[GET_LINK_AGE_VOID0];
        } else if (limbIndex == PLAYER_LIMB_L_HAND) {
            *dList = sFirstPersonLeftHandDLs[GET_LINK_AGE_VOID0];
        } else if (limbIndex == PLAYER_LIMB_R_SHOULDER) {
            *dList = sFirstPersonRightShoulderDLs[GET_LINK_AGE_VOID0];
        } else if (limbIndex == PLAYER_LIMB_R_FOREARM) {
            *dList = sFirstPersonForearmDLs[GET_LINK_AGE_VOID0];
        } else if (limbIndex == PLAYER_LIMB_R_HAND) {
            *dList = Player_HoldsHookshot(this)
                         ? gLinkAdultRightHandHoldingHookshotFarDL
                         : sFirstPersonRightHandHoldingWeaponDLs[GET_LINK_AGE_VOID0];
        } else {
            *dList = NULL;
        }
    }
...
void Player_PostLimbDrawGameplay(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->itemActionParam == PLAYER_AP_STICK) {
            ...
            gSPDisplayList(POLY_XLU_DISP++, sBottleDLists[(GET_LINK_AGE_VOID0)]);
        ...
        if (this->rightHandType == PLAYER_MODELTYPE_RH_FF) {
            ...
            BowSlingshotStringData* stringData = &sBowSlingshotStringData[GET_LINK_AGE];
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            ...
            Vec3f* footPos = &sLeftRightFootLimbModelFootPos[(GET_LINK_AGE_VOID0)];
...
u32 func_80091738(PlayState* play, u8* segment, SkelAnime* skelAnime) {
    s16 linkObjectId = gLinkObjectIds[GET_LINK_AGE_VOID0];
    u32 size;
    void* ptr;
    ...
    SkelAnime_InitLink(play, skelAnime, gPlayerSkelHeaders[GET_LINK_AGE_VOID0],
...
s32 Player_OverrideLimbDrawPause(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* arg) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        sLeftHandType = type;
        if ((type == PLAYER_MODELTYPE_LH_BGS) && (GET_SWORD_HEALTH <= 0.0f)) {
            dListOffset = 4;
        }
    } else if (limbIndex == PLAYER_LIMB_R_HAND) {
    ...
    dLists = sPlayerDListGroups[type] + (GET_LINK_AGE_VOID0);
