...
void Map_SetPaletteData(PlayState* play, s16 room) {
    ...
    osSyncPrintf("ＰＡＬＥＴＥセット 【 i=%x : room=%x 】Room_Inf[%d][4]=%x  ( map_palete_no = %d )\n", paletteIndex,
                 room, mapIndex, GET_SCENE_FLAGS[mapIndex].rooms, interfaceCtx->mapPaletteIndex);
    osSyncPrintf(VT_RST);
    ...
    switch (play->sceneId) {
        ...
        case SCENE_HAKADAN_BS:
            for (i = 0; i < gMapData->maxPaletteCount[mapIndex]; i++) {
                ...
                if ((room != 0xFF) && (GET_SCENE_FLAGS[mapIndex].rooms & gBitFlags[room])) {
...
void Map_InitRoomData(PlayState* play, s16 room) {
    ...
    if (room >= 0) {
        switch (play->sceneId) {
            ...
            case SCENE_JYASINBOSS:
            case SCENE_HAKADAN_BS:
                GET_SCENE_FLAGS[mapIndex].rooms |= gBitFlags[room];
                osSyncPrintf("ＲＯＯＭ＿ＩＮＦ＝%d\n", GET_SCENE_FLAGS[mapIndex].rooms);
...
void Map_Init(PlayState* play) {
    ...
    switch (play->sceneId) {
        ...
        case SCENE_GANON_TOU:
            ...
            R_MAP_INDEX = SET_MAP_INDEX(mapIndex)
            ...
            case SCENE_GANON_TOU:
                if (!R_MINIMAP_DISABLED) {
                    ...
                    if (((play->sceneId != SCENE_SPOT01) && (play->sceneId != SCENE_SPOT04) &&
                        ...
                        if ((gMapData->owEntranceFlag[sEntranceIconMapIndex] == 0xFFFF) ||
                            ((gMapData->owEntranceFlag[sEntranceIconMapIndex] != 0xFFFF) &&
                             (GET_INF_TABLE[INFTABLE_1AX_INDEX] &
                    ...
                    if ((play->sceneId == SCENE_SPOT08) &&
                        (GET_INF_TABLE[INFTABLE_1AX_INDEX] & gBitFlags[INFTABLE_1A9_SHIFT])) {
    ...
    if ((play->pauseCtx.state == 0) && (play->pauseCtx.debugState == 0)) {
        switch (play->sceneId) {
            ...
            case SCENE_ICE_DOUKUTO:
                ...
                GET_SCENE_FLAGS[mapIndex].floors |= gBitFlags[floor];
                VREG(30) = floor;
