...
void func_800645A0(PlayState* play, CutsceneContext* csCtx) {
    ...
    if ((GET_CUTSCENE_TRIGGER != 0) && (play->transitionTrigger == TRANS_TRIGGER_START)) {
        SET_CUTSCENE_TRIGGER(0)
    }

    if ((GET_CUTSCENE_TRIGGER != 0) && (csCtx->state == CS_STATE_IDLE)) {
    ...
    switch (cmd->base) {
        ...
        case 25:
            ...
            if ((GET_DAY_TIME) >= CLOCK_TIME(19, 0)) {
                SET_DAY_TIME(CLOCK_TIME(19, 0) - 1)
            }
            break;
        case 26:
            if ((GET_DAY_TIME < CLOCK_TIME(4, 30)) || (GET_DAY_TIME >= CLOCK_TIME(6, 30))) {
                if ((GET_DAY_TIME >= CLOCK_TIME(6, 30)) &&
                    (GET_DAY_TIME < CLOCK_TIME(16, 0))) {
                    play->envCtx.lightSettingOverride = 1;
                } else if ((GET_DAY_TIME >= CLOCK_TIME(16, 0)) &&
                           (GET_DAY_TIME <= CLOCK_TIME(18, 30))) {
        ...
        case 32:
            ...
            break;
        case 33:
            SET_SUNS_SONG_STATE(SUNSSONG_START)
            break;
        case 34:
            if (IS_DAY) {
                GET_DAY_TIME -= gTimeSpeed;
            } else {
                GET_DAY_TIME -= gTimeSpeed * 2;
            }
            break;
        case 35:
...
void func_80065134(PlayState* play, CutsceneContext* csCtx, CsCmdDayTime* cmd) {
    ...
    if (csCtx->frames == cmd->startFrame) {
        ...
        SET_DAY_TIME(temp1 + temp2)
        SET_SKYBOX_TIME(temp1 + temp2)
    }
}
...
void Cutscene_Command_Terminator(PlayState* play, CutsceneContext* csCtx, CsCmdBase* cmd) {
    ...
    if ((GET_GAME_MODE != GAMEMODE_NORMAL) && (GET_GAME_MODE != GAMEMODE_END_CREDITS) &&
        ...
        (GET_FILE_NUM != 0xFEDC) && (play->transitionTrigger == TRANS_TRIGGER_OFF)) {
    ...
    if ((csCtx->frames == cmd->startFrame) || (temp != 0) ||
        ...
        SET_CUTSCENE_TRANSITION_CONTROL(1)

        osSyncPrintf("\n分岐先指定！！=[%d]番", cmd->base); // "Future fork designation=No. [%d]"

        if ((GET_GAME_MODE != GAMEMODE_NORMAL) && (csCtx->frames != cmd->startFrame)) {
            SET_UNK_13_E7(1)
        }
        ...
        switch (cmd->base) {
            ...
            case 8:
                SET_FW_SET(0)
                GET_RESPAWN[RESPAWN_MODE_TOP].data = 0;
            ...
            case 30:
                ...
                SET_CHAMBER_CUTSCENE_NUM(1)
                break;
            case 31:
                play->nextEntranceIndex = ENTR_KENJYANOMA_0;
                play->transitionTrigger = TRANS_TRIGGER_START;
                play->transitionType = TRANS_TYPE_FADE_WHITE;
                SET_CHAMBER_CUTSCENE_NUM(2)
                break;
            case 32:
                play->linkAgeOnLoad = LINK_AGE_CHILD;
            ...
            case 48:
                ...
                SET_NEXT_TRANSITION_TYPE(TRANS_TYPE_SANDSTORM_END)
                break;
            case 49:
            ...
            case 53:
                ...
                break;
            case 54:
                SET_GAME_MODE(GAMEMODE_END_CREDITS)
            ...
            case 70:
                ...
                break;
            case 71:
                GET_EQUIPS_EQUIPMENT |= EQUIP_VALUE_TUNIC_KOKIRI << (EQUIP_TYPE_TUNIC * 4);
                Player_SetEquipmentData(play, player);
                GET_EQUIPS_EQUIPMENT |= EQUIP_VALUE_BOOTS_KOKIRI << (EQUIP_TYPE_BOOTS * 4);
                Player_SetEquipmentData(play, player);
                play->linkAgeOnLoad = LINK_AGE_CHILD;
                play->nextEntranceIndex = ENTR_TOKINOMA_0;
                play->transitionTrigger = TRANS_TRIGGER_START;
            ...
            case 117:
                ...
                break;
            case 118:
                GET_RESPAWN[RESPAWN_MODE_DOWN].entranceIndex = ENTR_GANON_DEMO_0;
                Play_TriggerVoidOut(play);
                SET_RESPAWN_FLAG(-2)
                SET_NEXT_TRANSITION_TYPE(TRANS_TYPE_FADE_BLACK)
                break;
            case 119:
                SET_DAY_TIME(CLOCK_TIME(12, 0))
                SET_SKYBOX_TIME(CLOCK_TIME(12, 0))
        ...
        switch (cmd->base) {
            ...
            case 5:
                ...
                if (cmd->base == 1) {
                    ...
                    if ((temp == 0.0f) && (GET_ENTRANCE_INDEX == ENTR_KENJYANOMA_0)) {
                        Audio_PlaySfxGeneral(NA_SE_SY_WHITE_OUT_S, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                    } else if ((temp == 0.0f) && ((GET_ENTRANCE_INDEX == ENTR_TOKINOMA_0) ||
                                                  (GET_ENTRANCE_INDEX == ENTR_SPOT15_0) ||
                                                  (GET_ENTRANCE_INDEX == ENTR_YOUSEI_IZUMI_YOKO_0))) {
            ...
            case 12:
                SET_CUTSCENE_TRANSITION_CONTROL(255.0f - (155.0f * temp))
                break;
...
void func_80068ECC(PlayState* play, CutsceneContext* csCtx) {
    ...
    if ((GET_CUTSCENE_TRIGGER != 0) && (csCtx->state == CS_STATE_IDLE) && !Player_InCsMode(play)) {
        SET_CUTSCENE_INDEX(0xFFFD)
    }

    if ((GET_CUTSCENE_INDEX >= 0xFFF0) && (csCtx->state == CS_STATE_IDLE)) {
...
void Cutscene_HandleConditionalTriggers(PlayState* play) {
    osSyncPrintf("\ngame_info.mode=[%d] restart_flag", (GET_RESPAWN_FLAG_VOID0));

    if ((GET_GAME_MODE == GAMEMODE_NORMAL) && (GET_RESPAWN_FLAG <= 0) &&
        (GET_CUTSCENE_INDEX < 0xFFF0)) {
        if ((GET_ENTRANCE_INDEX == ENTR_SPOT11_1) && !Flags_GetEventChkInf(EVENTCHKINF_AC)) {
            Flags_SetEventChkInf(EVENTCHKINF_AC);
            SET_ENTRANCE_INDEX(ENTR_SPOT11_0)
            SET_CUTSCENE_INDEX(0xFFF0)
        } else if ((GET_ENTRANCE_INDEX == ENTR_SPOT01_0) && LINK_IS_ADULT &&
            ...
            SET_CUTSCENE_INDEX(0xFFF0)
        } else if ((GET_ENTRANCE_INDEX == ENTR_SPOT10_9) && !Flags_GetEventChkInf(EVENTCHKINF_C1)) {
            Flags_SetEventChkInf(EVENTCHKINF_C1);
            Item_Give(play, ITEM_OCARINA_FAIRY);
            SET_ENTRANCE_INDEX(ENTR_SPOT10_0)
            SET_CUTSCENE_INDEX(0xFFF0)
        } else if (CHECK_QUEST_ITEM(QUEST_MEDALLION_SPIRIT) && CHECK_QUEST_ITEM(QUEST_MEDALLION_SHADOW) &&
                   LINK_IS_ADULT && !Flags_GetEventChkInf(EVENTCHKINF_C4) &&
                   (gEntranceTable[(GET_ENTRANCE_INDEX_VOID0)].sceneId == SCENE_TOKINOMA)) {
            Flags_SetEventChkInf(EVENTCHKINF_C4);
            SET_ENTRANCE_INDEX(ENTR_TOKINOMA_0)
            SET_CUTSCENE_INDEX(0xFFF8)
        } else if (!Flags_GetEventChkInf(EVENTCHKINF_C7) &&
                   (gEntranceTable[(GET_ENTRANCE_INDEX_VOID0)].sceneId == SCENE_GANON_DEMO)) {
            Flags_SetEventChkInf(EVENTCHKINF_C7);
            SET_ENTRANCE_INDEX(ENTR_GANON_DEMO_0)
            SET_CUTSCENE_INDEX(0xFFF0)
        }
    }
}
