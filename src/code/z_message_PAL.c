...
void Message_LoadItemIcon(PlayState* play, u16 itemId, s16 y) {
    ...
    if (itemId < ITEM_MEDALLION_FOREST) {
        R_TEXTBOX_ICON_XPOS = R_TEXT_INIT_XPOS - sIconItem32XOffsets[GET_LANGUAGE];
        ...
        R_TEXTBOX_ICON_XPOS = R_TEXT_INIT_XPOS - sIconItem24XOffsets[GET_LANGUAGE];
...
void Message_Decode(PlayState* play) {
    ...
    while (true) {
        ...
        if (curChar == MESSAGE_BOX_BREAK || curChar == MESSAGE_TEXTID || curChar == MESSAGE_BOX_BREAK_DELAYED ||
            ...
            for (playerNameLen = ARRAY_COUNT(GET_PLAYER_NAME); playerNameLen > 0;
                 playerNameLen--) {
                if (GET_PLAYER_NAME[playerNameLen - 1] != 0x3E) {
                    break;
                }
            }
            // "Name"
            ...
            if (curChar == MESSAGE_RACE_TIME) {
                digits[3] = GET_TIMER1_VALUE;
            } else {
                digits[3] = GET_TIMER2_VALUE;
            }
            ...
            osSyncPrintf("\n金スタ合計数 ＝ %d", GET_INVENTORY_GS_TOKENS);
            digits[0] = digits[1] = 0;
            digits[2] = GET_INVENTORY_GS_TOKENS;
            ...
            osSyncPrintf("\nゼルダ時間 ＝ ");
            digits[0] = 0;
            timeInSeconds = GET_DAY_TIME * (24.0f * 60.0f / 0x10000);
...
void Message_OpenText(PlayState* play, u16 textId) {
    ...
    if (textId == 0xC2 || textId == 0xFA) {
        ...
        textId += (GET_INVENTORY_QUEST_ITEMS & 0xF0000000 & 0xF0000000) >> QUEST_HEART_PIECE_COUNT;
    ...
    if (textId == 0x2030) { // Talking to Ingo as adult in Lon Lon Ranch for the first time before freeing Epona
        ...
        osSyncPrintf(VT_RST);
        GET_EVENT_INF[0] = GET_EVENT_INF[1] = GET_EVENT_INF[2] = GET_EVENT_INF[3] = 0;
    }
...
void Message_StartOcarina(PlayState* play, u16 ocarinaActionId) {
    ...
    if (noStop == false) {
        ...
        noStop = GET_UNK_13_E_A;
        Interface_ChangeAlpha(0xA);
        SET_UNK_13_E_A(noStop)
    }
        ...
        switch (msgCtx->msgMode) {
                ...
                if (msgCtx->ocarinaStaff->state == OCARINA_RECORD_OFF ||
                    ...
                    if (sOcarinaButtonIndexBufLen != 0) {
                        ...
                        osSyncPrintf("録音終了！！！！！！！！！  message->info->status=%d \n",
                                     msgCtx->ocarinaStaff->state);
                        SET_SCARECROW_LONG_SONG_SET(true)
                    }
                    Audio_PlaySfxGeneral(NA_SE_SY_OCARINA_ERROR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                    ...
                    osSyncPrintf(VT_FGCOL(YELLOW));
                    osSyncPrintf("\n====================================================================\n");
                    MemCpy(GET_SCARECROW_LONG_SONG, gScarecrowLongSongPtr,
                           sizeof(GET_SCARECROW_LONG_SONG));
                    for (i = 0; i < ARRAY_COUNT(GET_SCARECROW_LONG_SONG); i++) {
                        osSyncPrintf("%d, ", GET_SCARECROW_LONG_SONG[i]);
                    }
                    osSyncPrintf(VT_RST);
                    osSyncPrintf("\n====================================================================\n");
                }
            ...
            case MSGMODE_SCARECROW_SPAWN_RECORDING_ONGOING:
                ...
                if (msgCtx->ocarinaStaff->state == OCARINA_RECORD_OFF) {
                    ...
                    osSyncPrintf("８音録音ＯＫ！\n");
                    msgCtx->stateTimer = 20;
                    SET_SCARECROW_SPAWN_SONG_SET(true)
                    msgCtx->msgMode = MSGMODE_SCARECROW_SPAWN_RECORDING_DONE;
                    Audio_PlaySfxGeneral(NA_SE_SY_TRE_BOX_APPEAR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                    osSyncPrintf(VT_FGCOL(YELLOW));
                    osSyncPrintf("\n====================================================================\n");
                    MemCpy(GET_SCARECROW_SPAWN_SONG, gScarecrowSpawnSongPtr,
                           sizeof(GET_SCARECROW_SPAWN_SONG));
                    for (i = 0; i < ARRAY_COUNT(GET_SCARECROW_SPAWN_SONG); i++) {
                        osSyncPrintf("%d, ", GET_SCARECROW_SPAWN_SONG[i]);
                    }
                    osSyncPrintf(VT_RST);
                    osSyncPrintf("\n====================================================================\n");
            ...
            case MSGMODE_MEMORY_GAME_START:
                AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
                AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_FLUTE);
                AudioOcarina_MemoryGameInit(GET_OCARINA_GAME_ROUND_NUM);
                msgCtx->ocarinaStaff = AudioOcarina_GetPlaybackStaff();
                msgCtx->ocarinaStaff->pos = sOcarinaButtonIndexBufPos = 0;
                Message_ResetOcarinaNoteState();
                AudioOcarina_SetPlaybackSong(OCARINA_SONG_MEMORY_GAME + 1, 1);
...
void Message_Draw(PlayState* play) {
    ...
    watchVar = GET_SCARECROW_LONG_SONG_SET;
    ...
    switch (msgCtx->msgMode) {
        ...
        case MSGMODE_TEXT_CLOSING:
            ...
            if ((msgCtx->textId >= 0xC2 && msgCtx->textId < 0xC7) ||
                ...
                SET_HEALTH_ACCUMULATOR(0x140) // Refill 20 hearts
            }
            ...
            if (play->csCtx.state == 0) {
                osSyncPrintf(VT_FGCOL(GREEN));
                osSyncPrintf("day_time=%x  active_camera=%d  ", GET_CUTSCENE_INDEX, play->activeCamId);
                ...
                if (msgCtx->textId != 0x2061 && msgCtx->textId != 0x2025 && msgCtx->textId != 0x208C &&
                    ...
                    (msgCtx->textId != 0x3055 && GET_CUTSCENE_INDEX < 0xFFF0)) {
                    osSyncPrintf("=== day_time=%x ", (GET_CUTSCENE_INDEX_VOID0));
                    if (play->activeCamId == CAM_ID_MAIN) {
                        if (GET_UNK_13_E_E == 0 || GET_UNK_13_E_E == 1 || GET_UNK_13_E_E == 2) {
                            SET_UNK_13_E_E(0x32)
                        }
                        SET_UNK_13_E_A(0)
                        Interface_ChangeAlpha(GET_UNK_13_E_E);
                    }
                }
            }
            osSyncPrintf(VT_RST);
            ...
            if ((s32)(GET_INVENTORY_QUEST_ITEMS & 0xF0000000) == (4 << QUEST_HEART_PIECE_COUNT)) {
                GET_INVENTORY_QUEST_ITEMS ^= (4 << QUEST_HEART_PIECE_COUNT);
