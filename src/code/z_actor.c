...
void func_8002FA60(PlayState* play) {
    ...
    if (GET_FW_SET) {
        GET_RESPAWN[RESPAWN_MODE_TOP].data = 0x28;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.x = GET_FW_POS_X;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.y = GET_FW_POS_Y;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.z = GET_FW_POS_Z;
        GET_RESPAWN[RESPAWN_MODE_TOP].yaw = GET_FW_YAW;
        GET_RESPAWN[RESPAWN_MODE_TOP].playerParams = GET_FW_PLAYER_PARAMS;
        GET_RESPAWN[RESPAWN_MODE_TOP].entranceIndex = GET_FW_ENTRANCE_INDEX;
        GET_RESPAWN[RESPAWN_MODE_TOP].roomIndex = GET_FW_ROOM_INDEX;
        GET_RESPAWN[RESPAWN_MODE_TOP].tempSwchFlags = GET_FW_TEMP_SWCH_FLAGS;
        GET_RESPAWN[RESPAWN_MODE_TOP].tempCollectFlags = GET_FW_TEMP_COLLECT_FLAGS;
    } else {
        GET_RESPAWN[RESPAWN_MODE_TOP].data = 0;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.x = 0.0f;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.y = 0.0f;
        GET_RESPAWN[RESPAWN_MODE_TOP].pos.z = 0.0f;
    }

    lightPos.x = GET_RESPAWN[RESPAWN_MODE_TOP].pos.x;
    lightPos.y = GET_RESPAWN[RESPAWN_MODE_TOP].pos.y + 80.0f;
    lightPos.z = GET_RESPAWN[RESPAWN_MODE_TOP].pos.z;
...
void Actor_DrawFaroresWindPointer(PlayState* play) {
    ...
    params = GET_RESPAWN[RESPAWN_MODE_TOP].data;
    ...
    if (params) {
        ...
        if (temp < 0) {
            GET_RESPAWN[RESPAWN_MODE_TOP].data = ++params;
            ...
            Vec3f* curPos = &GET_RESPAWN[RESPAWN_MODE_TOP].pos;
            Vec3f* nextPos = &GET_RESPAWN[RESPAWN_MODE_DOWN].pos;
            ...
            if (D_8015BC18 == 0.0f) {
                GET_RESPAWN[RESPAWN_MODE_TOP] = GET_RESPAWN[RESPAWN_MODE_DOWN];
                GET_RESPAWN[RESPAWN_MODE_TOP].playerParams = 0x06FF;
                GET_RESPAWN[RESPAWN_MODE_TOP].data = 40;
            }

            GET_RESPAWN[RESPAWN_MODE_TOP].pos = *curPos;
        } else if (temp > 0) {
            Vec3f* curPos = &GET_RESPAWN[RESPAWN_MODE_TOP].pos;
        ...
        if ((play->csCtx.state == CS_STATE_IDLE) &&
            ((GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].entranceIndex) ==
             (GET_ENTRANCE_INDEX_VOID0)) &&
            ((GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].roomIndex) == play->roomCtx.curRoom.num)) {
            f32 scale = 0.025f * ratio;

            POLY_XLU_DISP = Gfx_SetupDL(POLY_XLU_DISP, SETUPDL_25);

            Matrix_Translate((GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.x),
                             (GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.y) + yOffset,
                             (GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.z), MTXMODE_NEW);
        ...
        Lights_PointNoGlowSetInfo(&D_8015BC00, (GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.x),
                                  (GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.y) + yOffset,
                                  (GET_RESPAWN_VOID0[RESPAWN_MODE_TOP].pos.z), 255, 255, 255, lightRadius);
...
void func_800304DC(PlayState* play, ActorContext* actorCtx, ActorEntry* actorEntry) {
    ...
    savedSceneFlags = &GET_SCENE_FLAGS[play->sceneId];
    ...
    switch (arg1) {
        ...
        case 16:
            if (play->sceneId == SCENE_SPOT15) {
                ...
                retTextId = 0x7004;
            } else if ((GET_DAY_TIME >= CLOCK_TIME(6, 0)) &&
...
s32 Actor_TrackPlayerSetFocusHeight(PlayState* play, Actor* actor, Vec3s* headRot, Vec3s* torsoRot, f32 focusHeight) {
    ...
    if (!(((play->csCtx.state != CS_STATE_IDLE) || gDbgCamEnabled) &&
          (GET_ENTRANCE_INDEX == ENTR_SPOT04_0))) {
