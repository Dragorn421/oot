...
void func_8006D074(PlayState* play) {
    ...
    SET_HORSE_DATA_ANGLE(-27353)
}

void func_8006D0AC(PlayState* play) {
    if (GET_HORSE_DATA_SCENE_ID == SCENE_SPOT06) {
        SET_HORSE_DATA_SCENE_ID(SCENE_SPOT06)
        SET_HORSE_DATA_POS_X(-2065)
        SET_HORSE_DATA_POS_Y(-863)
        SET_HORSE_DATA_POS_Z(1839)
        SET_HORSE_DATA_ANGLE(0)
    }
}
...
void func_8006D0EC(PlayState* play, Player* player) {
    ...
    if ((AREG(6) != 0) && (Flags_GetEventChkInf(EVENTCHKINF_18) || (DREG(1) != 0))) {
        ...
        SET_HORSE_DATA_SCENE_ID(play->sceneId)

        if (play->sceneId == SCENE_SPOT12) {
            player->rideActor->room = -1;
        }
    } else if ((play->sceneId == SCENE_SPOT12) && (GET_MINIGAME_STATE == 3)) {
        Actor* horseActor;
        SET_MINIGAME_STATE(0)
        horseActor = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 3586.0f, 1413.0f, -402.0f, 0, 0x4000, 0, 1);
        horseActor->room = -1;
    } else if ((GET_ENTRANCE_INDEX == ENTR_SPOT20_7) && GET_EVENTCHKINF(EVENTCHKINF_18)) {
        Actor* horseActor =
    ...
    } else if ((play->sceneId == GET_HORSE_DATA_SCENE_ID) &&
               (Flags_GetEventChkInf(EVENTCHKINF_18) || DREG(1) != 0)) {
        // "Set by existence of horse %d %d %d"
        osSyncPrintf("馬存在によるセット %d %d %d\n", GET_HORSE_DATA_SCENE_ID,
                     Flags_GetEventChkInf(EVENTCHKINF_18), DREG(1));

        if (func_8006CFC0(GET_HORSE_DATA_SCENE_ID)) {
            Actor* horseActor =
                Actor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, GET_HORSE_DATA_POS_X,
                            GET_HORSE_DATA_POS_Y, GET_HORSE_DATA_POS_Z, 0,
                            GET_HORSE_DATA_ANGLE, 0, 1);
...
void func_8006D684(PlayState* play, Player* player) {
    ...
    if ((GET_ENTRANCE_INDEX == ENTR_SPOT00_11 || GET_ENTRANCE_INDEX == ENTR_SPOT00_12 ||
         GET_ENTRANCE_INDEX == ENTR_SPOT00_13 || GET_ENTRANCE_INDEX == ENTR_SPOT00_15) &&
        (GET_RESPAWN_FLAG == 0)) {
        ...
        if (GET_ENTRANCE_INDEX == ENTR_SPOT00_11) {
            spawnPos = spawnPositions[0];
        } else if (GET_ENTRANCE_INDEX == ENTR_SPOT00_12) {
            spawnPos = spawnPositions[1];
        } else if (GET_ENTRANCE_INDEX == ENTR_SPOT00_13) {
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011F9B8); i++) {
            if ((play->sceneId == D_8011F9B8[i].sceneId) &&
                ((GET_CUTSCENE_INDEX_VOID0) == D_8011F9B8[i].cutsceneIndex)) {
                if (D_8011F9B8[i].type == 7) {
                    if ((play->sceneId == SCENE_SPOT20) && ((GET_CUTSCENE_INDEX_VOID0) == 0xFFF1)) {
    ...
    if (LINK_IS_ADULT) {
        ...
        if (func_8006CFC0(play->sceneId)) {
            if (IS_CUTSCENE_LAYER ||
                ((GET_ENTRANCE_INDEX == ENTR_SPOT00_11 ||
