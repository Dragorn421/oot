...
void func_8006D074(PlayState* play) {
    ...
    SET_HORSEDATA_ANGLE(-27353);
}

void func_8006D0AC(PlayState* play) {
    if (GET_HORSEDATA_SCENEID == SCENE_SPOT06) {
        SET_HORSEDATA_SCENEID(SCENE_SPOT06);
        SET_HORSEDATA_POS_X(-2065);
        SET_HORSEDATA_POS_Y(-863);
        SET_HORSEDATA_POS_Z(1839);
        SET_HORSEDATA_ANGLE(0);
    }
}
...
void func_8006D0EC(PlayState* play, Player* player) {
    ...
    } else if ((play->sceneId == SCENE_SPOT12) && (GET_MINIGAMESTATE == 3)) {
        Actor* horseActor;
        SET_MINIGAMESTATE(0);
        horseActor = Actor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, 3586.0f, 1413.0f, -402.0f, 0, 0x4000, 0, 1);
        horseActor->room = -1;
    } else if ((GET_ENTRANCEINDEX == ENTR_SPOT20_7) && GET_EVENTCHKINF(EVENTCHKINF_18)) {
        Actor* horseActor =
...
void func_8006D684(PlayState* play, Player* player) {
    ...
    if ((GET_ENTRANCEINDEX == ENTR_SPOT00_11 || GET_ENTRANCEINDEX == ENTR_SPOT00_12 ||
        ...
        if (GET_ENTRANCEINDEX == ENTR_SPOT00_11) {
            spawnPos = spawnPositions[0];
        } else if (GET_ENTRANCEINDEX == ENTR_SPOT00_12) {
            spawnPos = spawnPositions[1];
        } else if (GET_ENTRANCEINDEX == ENTR_SPOT00_13) {
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011F9B8); i++) {
            if ((play->sceneId == D_8011F9B8[i].sceneId) &&
                ((GET_CUTSCENEINDEX_VOID0) == D_8011F9B8[i].cutsceneIndex)) {
                if (D_8011F9B8[i].type == 7) {
                    if ((play->sceneId == SCENE_SPOT20) && ((GET_CUTSCENEINDEX_VOID0) == 0xFFF1)) {
