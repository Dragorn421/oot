...
void Environment_Init(PlayState* play2, EnvironmentContext* envCtx, s32 unused) {
    ...
    SET_CUTSCENETRANSITIONCONTROL(0);
    ...
    if (CREG(3) != 0) {
        SET_CHAMBERCUTSCENENUM(CREG(3) - 1);
    }
...
void Environment_UpdateSkybox(u8 skyboxId, EnvironmentContext* envCtx, SkyboxContext* skyboxCtx) {
    ...
    if (skyboxId == SKYBOX_CUTSCENE_MAP) {
        ...
        for (i = 0; i < ARRAY_COUNT(gTimeBasedSkyboxConfigs[envCtx->skyboxConfig]); i++) {
            if (GET_SKYBOXTIME >= gTimeBasedSkyboxConfigs[envCtx->skyboxConfig][i].startTime &&
                (GET_SKYBOXTIME < gTimeBasedSkyboxConfigs[envCtx->skyboxConfig][i].endTime ||
...
void Environment_PrintDebugInfo(PlayState* play, Gfx** gfx) {
    ...
    GfxPrint_Printf(&printer, "T%03d ", (GET_TOTALDAYS_VOID0));
    GfxPrint_Printf(&printer, "E%03d", (GET_BGSDAYCOUNT_VOID0));
    ...
    GfxPrint_Printf(&printer, "%02d", (u8)(24 * 60 / (f32)0x10000 * (GET_DAYTIME_VOID0) / 60.0f));

    if ((GET_DAYTIME & 0x1F) >= 0x10 || gTimeSpeed >= 6) {
        GfxPrint_Printf(&printer, "%s", ":");
    } else {
        GfxPrint_Printf(&printer, "%s", " ");
    }

    GfxPrint_Printf(&printer, "%02d", (s16)(24 * 60 / (f32)0x10000 * (GET_DAYTIME_VOID0)) % 60);
    ...
    GfxPrint_Printf(&printer, "%02d", (s16)(24 * 60 / (f32)0x10000 * (GET_SKYBOXTIME_VOID0)) % 60);
...
void Environment_Update(PlayState* play, EnvironmentContext* envCtx, LightContext* lightCtx, PauseContext* pauseCtx,
    ...
    if (pauseCtx->state == 0) {
        ...
        if ((GET_NEXTDAYTIME_VOID0) >= 0xFF00 && (GET_NEXTDAYTIME_VOID0) != NEXT_TIME_NONE) {
            GET_NEXTDAYTIME -= 0x10;
            osSyncPrintf("\nnext_zelda_time=[%x]", (GET_NEXTDAYTIME_VOID0));
            ...
            if ((GET_NEXTDAYTIME_VOID0) == (NEXT_TIME_DAY_SET - (15 * 0x10))) {
                func_80078884(NA_SE_EV_CHICKEN_CRY_M);
                SET_NEXTDAYTIME(NEXT_TIME_NONE);
            } else if ((GET_NEXTDAYTIME_VOID0) == (NEXT_TIME_NIGHT_SET - (15 * 0x10))) {
        ...
        if ((pauseCtx->state == 0) && (gameOverCtx->state == GAMEOVER_INACTIVE)) {
            if (((msgCtx->msgLength == 0) && (msgCtx->msgMode == MSGMODE_NONE)) ||
                ...
                if ((envCtx->changeSkyboxTimer == 0) && !FrameAdvance_IsEnabled(play) &&
                    (play->transitionMode == TRANS_MODE_OFF || (GET_GAMEMODE_VOID0) != GAMEMODE_NORMAL)) {

                    if (IS_DAY || gTimeSpeed >= 400) {
                        GET_DAYTIME += gTimeSpeed;
                    } else {
                        GET_DAYTIME += gTimeSpeed * 2; // time moves twice as fast at night
                    }
                }
            }
        }
        ...
        if (envCtx->lightSettingOverride != LIGHT_SETTING_OVERRIDE_FULL_CONTROL) {
            if ((envCtx->lightMode == LIGHT_MODE_TIME) &&
                ...
                for (i = 0; i < ARRAY_COUNT(sTimeBasedLightConfigs[envCtx->lightConfig]); i++) {
                    if ((GET_SKYBOXTIME >= sTimeBasedLightConfigs[envCtx->lightConfig][i].startTime) &&
                        ((GET_SKYBOXTIME < sTimeBasedLightConfigs[envCtx->lightConfig][i].endTime) ||
...
void Environment_PlaySceneSequence(PlayState* play) {
    ...
    if ((GET_ENTRANCEINDEX_VOID0) == ENTR_SPOT10_8 ||
        ...
        if (!Environment_IsForcedSequenceDisabled()) {
            Audio_QueueSeqCmd(SEQ_PLAYER_BGM_MAIN << 24 | (s32)(GET_FORCEDSEQID_VOID0));
        }
        ...
        if ((GET_NATUREAMBIENCEID_VOID0) != play->sequenceCtx.natureAmbienceId) {
    ...
    osSyncPrintf("\n-----------------\n", (GET_FORCEDSEQID_VOID0));
    osSyncPrintf("\n 強制ＢＧＭ=[%d]", (GET_FORCEDSEQID_VOID0)); // "Forced BGM"
...
void Environment_PlayTimeBasedSequence(PlayState* play) {
    switch (play->envCtx.timeSeqState) {
        ...
        case TIMESEQ_FADE_DAY_BGM:
            if (GET_DAYTIME > CLOCK_TIME(17, 10)) {
        ...
        case TIMESEQ_DAY_BEGIN_SFX:
            if ((GET_DAYTIME <= CLOCK_TIME(19, 0)) && (GET_DAYTIME > CLOCK_TIME(6, 30))) {
                GET_TOTALDAYS++;
                GET_BGSDAYCOUNT++;
                SET_DOGISLOST(true);
                func_80078884(NA_SE_EV_CHICKEN_CRY_M);
...
void Environment_StopStormNatureAmbience(PlayState* play) {
    ...
    if (func_800FA0B4(SEQ_PLAYER_BGM_MAIN) == NA_BGM_NATURE_AMBIENCE) {
        SET_SEQID(NA_BGM_NATURE_SFX_RAIN);
...
void Environment_WarpSongLeave(PlayState* play) {
    ...
    SET_CUTSCENEINDEX(0);
    SET_RESPAWNFLAG(-3);
    play->nextEntranceIndex = GET_RESPAWN_A0_ENTRANCEINDEX(RESPAWN_MODE_RETURN);
