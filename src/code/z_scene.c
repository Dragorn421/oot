...
void Scene_CommandSpawnList(PlayState* play, SceneCmd* cmd) {
    ...
    play->linkAgeOnLoad = (GET_LINK_AGE_VOID0);

    linkObjectId = gLinkObjectIds[(GET_LINK_AGE_VOID0)];
...
void Scene_CommandTimeSettings(PlayState* play, SceneCmd* cmd) {
    ...
    if (((play->envCtx.sceneTimeSpeed == 0) && (GET_CUTSCENE_INDEX < 0xFFF0)) ||
        (GET_ENTRANCE_INDEX == ENTR_SPOT06_8)) {
        SET_SKYBOX_TIME(((void)0, gSaveContext.save.dayTime))

        if ((GET_SKYBOX_TIME > CLOCK_TIME(4, 0)) && (GET_SKYBOX_TIME < CLOCK_TIME(6, 30))) {
            SET_SKYBOX_TIME(CLOCK_TIME(5, 0) + 1)
        } else if ((GET_SKYBOX_TIME >= CLOCK_TIME(6, 30)) && (GET_SKYBOX_TIME <= CLOCK_TIME(8, 0))) {
            SET_SKYBOX_TIME(CLOCK_TIME(8, 0) + 1)
        } else if ((GET_SKYBOX_TIME >= CLOCK_TIME(16, 0)) && (GET_SKYBOX_TIME <= CLOCK_TIME(17, 0))) {
            SET_SKYBOX_TIME(CLOCK_TIME(17, 0) + 1)
        } else if ((GET_SKYBOX_TIME >= CLOCK_TIME(18, 0) + 1) &&
                   (GET_SKYBOX_TIME <= CLOCK_TIME(19, 0))) {
            SET_SKYBOX_TIME(CLOCK_TIME(19, 0) + 1)
        }
    }
}
...
void Scene_CommandSoundSettings(PlayState* play, SceneCmd* cmd) {
    ...
    if (GET_SEQ_ID == (u8)NA_BGM_DISABLED) {
...
void Scene_CommandAlternateHeaderList(PlayState* play, SceneCmd* cmd) {
    ...
    osSyncPrintf("\n[ZU]sceneset age    =[%X]", (GET_LINK_AGE_VOID0));
    osSyncPrintf("\n[ZU]sceneset time   =[%X]", (GET_CUTSCENE_INDEX_VOID0));
    osSyncPrintf("\n[ZU]sceneset counter=[%X]", (GET_SCENE_LAYER_VOID0));

    if (GET_SCENE_LAYER != 0) {
        altHeader = ((SceneCmd**)SEGMENTED_TO_VIRTUAL(cmd->altHeaders.data))[GET_SCENE_LAYER - 1];
        ...
        if (altHeader != NULL) {
            ...
            if (GET_SCENE_LAYER == SCENE_LAYER_ADULT_NIGHT) {
                ...
                altHeader = ((SceneCmd**)SEGMENTED_TO_VIRTUAL(
                    cmd->altHeaders
                        .data))[(GET_SCENE_LAYER - SCENE_LAYER_ADULT_NIGHT) + SCENE_LAYER_ADULT_DAY - 1];
...
void Scene_CommandMiscSettings(PlayState* play, SceneCmd* cmd) {
    ...
    if (((play->sceneId >= SCENE_SPOT00) && (play->sceneId <= SCENE_GANON_TOU)) ||
        ...
        if (GET_CUTSCENE_INDEX < 0xFFF0) {
            GET_WORLD_MAP_AREA_DATA |= gBitFlags[GET_WORLD_MAP_AREA];
            osSyncPrintf("０００  ａｒｅａ＿ａｒｒｉｖａｌ＝%x (%d)\n", GET_WORLD_MAP_AREA_DATA,
                         GET_WORLD_MAP_AREA);
        }
    }
}
