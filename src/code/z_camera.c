...
s32 Camera_KeepOn3(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        if (!(keep3->flags & 0x80)) {
            while (i < angleCnt) {
                if (!func_800626B0(camera->globalCtx, &camera->globalCtx->colliderCtx, &anim->atTarget, &lineChkPointB,
                                   &colChkActors, 2) &&
        ...
        case 0x14:
            ...
            if (!(keep4->unk_1C & 1)) {
                ...
                for (i = 0; i < angleCnt; i++) {
                    if (!func_800626B0(camera->globalCtx, &camera->globalCtx->colliderCtx, &D_8015BD50, &D_8015BD70, spCC,
                                       sp9C) &&
