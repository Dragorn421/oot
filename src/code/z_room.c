...
void Room_Draw1Single(PlayState* play, Room* room, u32 flags) {
    ...
    drawBg = (flags & 1) && isFixedCamera && meshHeader1Single->source && !(SREG(25) & 1);
    drawOpa = (flags & 1) && meshHeader1Entry->opa != NULL && !(SREG(25) & 2);
    drawXlu = (flags & 2) && meshHeader1Entry->xlu != NULL && !(SREG(25) & 4);
